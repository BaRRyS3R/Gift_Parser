...
import { SurvivalGameState, SurvivalGameResult } from '@/types/game-modes/survival'
import {
...
export default function SurvivalGameManager({ onBackToMenu }: SurvivalGameManagerProps) {
    const { saveGameResult } = useUser()
    const [gameState, setGameState] = useState<SurvivalGameState>(initializeSurvivalGameState())
    ...
    const endGame = useCallback((cause: 'miss' | 'wrong_click' | 'decoy_hit') => {
    ...
    const scheduleNextActivation = useCallback(() => {
        ...
        const delay = Math.random() * (levelConfig.activationTimeMax - levelConfig.activationTimeMin) + levelConfig.activationTimeMin
                    ...
                    const newState = activateSurvivalCircles(
                        ...
                        (circleId: number, wasDecoy: boolean) => {
                            ...
                            if (!wasDecoy) {
                                ...
                                setGameState(current => deactivateSurvivalCircle(current, circleId))
    ...
    const handleCircleClickEvent = useCallback((circleId: number) => {
        ...
        const { newState, result } = handleSurvivalCircleClick(gameStateRef.current, circleId)
    ...
    if (gameState.gameState === GameState.FINISHED && gameResult) {
        const getDeathCauseIcon = () => {
            switch (gameResult.deathCause) {
                ...
                default: return <Crosshair size={20} className="text-red-400" />
            }
        }

        const getDeathCauseMessage = () => {
            switch (gameResult.deathCause) {
                case 'miss': return 'Failed to hit a white target in time'
                case 'wrong_click': return 'Clicked an inactive target'
                case 'decoy_hit': return 'Clicked a red trap circle'
                default: return 'Survival ended'
            }
        }
                ...
                <div className="w-full max-w-md space-y-8 animate-fade-in">
                    ...
                    <div className="bg-red-500/10 backdrop-blur-sm border border-red-400/30 rounded-xl p-6 space-y-6">
                        <div className="text-center space-y-2">
                            <div className="text-sm font-bpdots text-red-400/60">SURVIVAL TIME</div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="text-center space-y-1">
                                <div className="text-xs font-bpdots text-red-400/60">FINAL SCORE</div>
                                <div className="text-xl font-bold font-bpdots text-red-300">
                                    {gameResult.score}
                                </div>
                            </div>
                            <div className="text-center space-y-1">
                                <div className="text-xs font-bpdots text-red-400/60">PERFECT STREAK</div>
                                <div className="text-xl font-bold font-bpdots text-green-400">
                                    {gameResult.perfectStreak}
                                </div>
                            </div>
                            <div className="text-center space-y-1">
                                <div className="text-xs font-bpdots text-red-400/60">CORRECT HITS</div>
                                <div className="text-xl font-bold font-bpdots text-green-400">
                        ...
                        <div className="border-t border-red-400/30 pt-4">
                            <div className="text-center space-y-2">
                                <div className="text-xs font-bpdots text-red-400/60 uppercase">Level Progress</div>
                                <div className="w-full h-2 bg-red-900/20 rounded-full overflow-hidden border border-red-400/30">
                                    <div
                                        ...
                                        style={{ width: `${Math.min(100, (gameResult.maxLevelReached / 15) * 100)}%` }}
                                    />
                                </div>
        ...
        <div className="min-h-screen bg-black flex flex-col text-white">
            ...
            <div className="flex items-center justify-between px-6 py-4 pt-20 z-10">
                <div className="flex flex-col items-center">
                    <div className="text-2xl font-bpdots text-red-400">
                        SURVIVAL
                    </div>
