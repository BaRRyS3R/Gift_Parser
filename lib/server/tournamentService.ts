...
function transformTournament(rawTournament: RawTournament): Tournament {
    ...
    const transformedPrizes: Prize[] = rawTournament.prizes.map((prize) => ({
        position: typeof prize.place === 'string' ?
            (prize.place === '4-10' ? 4 : parseInt(prize.place)) :
            prize.place,
        ...
        attempts: prize.prize.includes('attempts') ?
            ...
            undefined,
        reward_type: prize.prize.includes('attempts') ? 'attempts' as const : 'custom' as const,
    }));
    ...
    async getTournamentByQuery(query: string): Promise<Tournament | null> {
        try {
            ...
            const { data, error } = await supabaseServer
                .from("tournaments")
                .select("*")
                .eq("game_mode", mode) // Note: using game_mode here for database query
                .order("start_time", { ascending: false })
                .limit(10);
            ...
            const rawTournament = data?.[0] || null;

            if (!rawTournament) {
                console.log("No tournament found for query:", query);
                return null;
            }
