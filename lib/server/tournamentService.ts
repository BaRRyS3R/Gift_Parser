...
import {
    ...
    TournamentResult,
    TournamentFilters,
    TournamentStatus, // Regular import, not 'import type'
    TournamentGameMode,
...
export const serverTournamentService = {
    ...
    async getTournaments(filters: TournamentFilters = {}): Promise<Tournament[]> {
        try {
            ...
            const tournaments = (data || []).map(tournament => ({
                ...tournament,
                status: getTournamentStatusFromDates(tournament.start_date, tournament.end_date)
            }));
     ...
     */
    async getTournamentDetails(tournamentId: string, userId?: string): Promise<TournamentDetails> {
        try {
    ...
    async getTournamentLeaderboard(
        ...
        try {
            ...
            // Получаем лидерборд
            const { data: leaderboardData, error: leaderboardError } = await supabaseServer
                .from("tournament_results")
                .select(`
                    *,
                    users!inner(
            ...
            // Формируем лидерборд
            const leaderboard: TournamentLeaderboardEntry[] = (leaderboardData || []).map((entry, index) => ({
                position: index + 1,
            ...
            const totalGamesPlayed = statsData?.reduce((sum, result) => sum + result.games_played, 0) || 0;
            const highestScore = Math.max(...(statsData?.map(r => r.best_score) || [0]));
            const averageScore = totalParticipants > 0
                ? (statsData?.reduce((sum, result) => sum + result.best_score, 0) || 0) / totalParticipants
                : 0;
     ...
     */
    async canUserParticipate(tournamentId: string, userId: string): Promise<boolean> {
        try {
            ...
            const currentStatus = getTournamentStatusFromDates(tournament.start_date, tournament.end_date);
    ...
    async submitTournamentResult(
        ...
        try {
            ...
            const canParticipate = await this.canUserParticipate(tournamentId, userId);
            if (!canParticipate) {
            ...
            if (existingResult) {
                ...
                if (error) {
                    throw new Error(`Failed to create tournament result: ${error.message}`);
                }
