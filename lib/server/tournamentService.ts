...
export const serverTournamentService = {
  ...
  async saveTournamentResult(
    ...
    try {
      console.log(
        `Saving tournament result for user ${telegramId}, tournament ${tournamentId}:`,
        {
      ...
      if (existingParticipant) {
        ...
        const { data: newParticipant, error: createError } =
          await supabaseServer
            .from("tournament_participants")
            .insert({
      ...
      const { data: gameResultRecord, error: resultError } =
        await supabaseServer
      ...
      console.log(
        `Successfully saved tournament result for user ${telegramId}`,
      );
