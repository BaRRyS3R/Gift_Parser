// src/lib/server/tournamentService.ts - Server-side tournament management service
...
export const serverTournamentService = {
    ...
    /**
     * Get all tournaments with categorization by status
     */
    ...
    async getTournamentLeaderboard(tournamentId: string, limit: number = 50): Promise<TournamentLeaderboardEntry[]> {
        try {
            const { data, error } = await supabaseServer.rpc('get_tournament_leaderboard_accumulative', {
    ...
    /**
     * Get user tournament result and rank
     */
    async getUserTournamentResult(tournamentId: string, userId: string): Promise<TournamentResult | null> {
        try {
            const { data, error } = await supabaseServer.rpc('get_user_tournament_result', {
                tournament_id_param: tournamentId,
