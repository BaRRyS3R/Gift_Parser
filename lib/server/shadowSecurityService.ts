...
export class ShadowSecurityService {
    ...
    static async submitSuspiciousActivity(
        ...
        try {
            ...
            if (activityData.totalClicks <= 0 && activityData.suspiciousClicksCount > 0) {
                return {
                    success: false,
            ...
            if (activityData.gyroscopeEnabled) {
                if (activityData.gyroscopeMovementsDetected > activityData.totalGyroscopeChecks) {
                    return {
                        success: false,
                ...
                if (activityData.totalGyroscopeChecks > 0) {
                    const calculatedPercentage = (activityData.gyroscopeMovementsDetected / activityData.totalGyroscopeChecks) * 100;
            ...
            const record: Omit<SuspiciousActivityRecord, "id" | "created_at" | "updated_at"> = {
                ...
                gyroscope_check_intervals_ms: JSON.stringify(activityData.gyroscopeCheckIntervals),
            ...
            if (error) {
                console.error("Error inserting enhanced suspicious activity record:", error);
                return {
                    success: false,
            ...
            if (activityData.suspiciousClicksCount > 0) {
                const clickSuspiciousPercentage = Math.round((activityData.suspiciousClicksCount / activityData.totalClicks) * 100);
                logDetails.push(
            ...
            if (activityData.gyroscopeEnabled) {
                if (activityData.gyroscopeErrorReason) {
                    logDetails.push(`gyroscope error: ${activityData.gyroscopeErrorReason}`);
                } else {
                    logDetails.push(
    ...
    static async getUserSuspiciousActivity(
        ...
        try {
            ...
            if (error) {
                console.error("Error fetching enhanced suspicious activity records:", error);
                return {
                    success: false,
    ...
    static async getSuspiciousActivityStats(
        ...
        try {
            ...
            if (error) {
                console.error("Error fetching enhanced suspicious activity stats:", error);
                return {
                    success: false,
            ...
            const clickSuspiciousRecords = data.filter(record => record.suspicious_clicks_count > 0);
            const clickSuspiciousGames = clickSuspiciousRecords.length;

            const clickSuspiciousPercentages = clickSuspiciousRecords.map(
                record => (record.suspicious_clicks_count / Math.max(record.total_clicks, 1)) * 100
            );
            const averageClickSuspiciousPercentage = clickSuspiciousPercentages.length > 0
                ? Math.round((clickSuspiciousPercentages.reduce((sum, percentage) => sum + percentage, 0) /
                    clickSuspiciousPercentages.length) * 100) / 100
                : 0;

            // Gyroscope-based statistics
            const gyroscopeEnabledRecords = data.filter(record => record.gyroscope_enabled);
            const gyroscopeEnabledGames = gyroscopeEnabledRecords.length;
            const gyroscopeSuspiciousGames = data.filter(record => record.gyroscope_suspicious).length;

            const gyroscopeMovementPercentages = gyroscopeEnabledRecords.map(
                record => record.gyroscope_movement_percentage || 0
            );
            const averageGyroscopeMovementPercentage = gyroscopeMovementPercentages.length > 0
                ? Math.round((gyroscopeMovementPercentages.reduce((sum, percentage) => sum + percentage, 0) /
                    gyroscopeMovementPercentages.length) * 100) / 100
                : 0;

            // Combined statistics
            const combinedSuspiciousGames = data.filter(record =>
            ...
            const overallSuspiciousPercentages = data.map(record => {
                ...
                if (record.total_clicks > 0) {
                    combinedSuspiciousness += (record.suspicious_clicks_count / record.total_clicks) * 100;
                    factorsCount++;
                }
                ...
                if (record.gyroscope_enabled && record.total_gyroscope_checks > 0) {
                    const gyroscopeSuspiciousness = Math.max(0, 100 - (record.gyroscope_movement_percentage || 0));
            ...
            const averageSuspiciousPercentage = overallSuspiciousPercentages.length > 0
                ? Math.round((overallSuspiciousPercentages.reduce((sum, percentage) => sum + percentage, 0) /
                    overallSuspiciousPercentages.length) * 100) / 100
                : 0;
            ...
            data.forEach((record) => {
                ...
                userStatsMap.set(userId, {
                    clickSuspicious: existing.clickSuspicious + record.suspicious_clicks_count,
                    clickTotal: existing.clickTotal + record.total_clicks,
                    gyroscopeMovementSum: existing.gyroscopeMovementSum +
                        (record.gyroscope_enabled ? (record.gyroscope_movement_percentage || 0) : 0),
                    gyroscopeGames: existing.gyroscopeGames + (record.gyroscope_enabled ? 1 : 0),
            ...
            const mostSuspiciousUsers = Array.from(userStatsMap.entries())
                .map(([telegram_id, stats]) => {
                    const clickSuspiciousPercentage = stats.clickTotal > 0
                        ? Math.round((stats.clickSuspicious / stats.clickTotal) * 10000) / 100
                        : 0;

                    const gyroscopeMovementPercentage = stats.gyroscopeGames > 0
                        ? Math.round((stats.gyroscopeMovementSum / stats.gyroscopeGames) * 100) / 100
                ...
                .filter(user => user.suspicious_percentage > 0 || user.gyroscope_movement_percentage < 90)
            ...
            data.forEach(record => {
                if (record.gyroscope_error_reason) {
                    const count = gyroscopeErrorMap.get(record.gyroscope_error_reason) || 0;
    ...
    static async hasRecentSuspiciousActivity(
        ...
        try {
            ...
            if (error) {
                console.error("Error checking enhanced recent suspicious activity:", error);
                return {
                    success: false,
            ...
            const totalSuspiciousClicks = data.reduce((sum, record) => sum + record.suspicious_clicks_count, 0);
            const totalClicks = data.reduce((sum, record) => sum + record.total_clicks, 0);
            const clickSuspiciousPercentage = totalClicks > 0
            ...
            const totalGyroscopeMovement = gyroscopeEnabledRecords.reduce(
                (sum, record) => sum + (record.gyroscope_movement_percentage || 0), 0
            );
            const gyroscopeAverageMovement = gyroscopeEnabledGames > 0
                ? Math.round((totalGyroscopeMovement / gyroscopeEnabledGames) * 100) / 100
                : 100;

            // Enhanced risk assessment
            const hasClickSuspicion = clickSuspiciousPercentage >= clickSuspiciousThreshold;
            const hasGyroscopeSuspicion = gyroscopeEnabledGames > 0 && gyroscopeAverageMovement <= gyroscopeMovementThreshold;

            // Combined risk score calculation
            const clickRisk = clickSuspiciousPercentage * 0.6;
            const gyroscopeRisk = gyroscopeEnabledGames > 0 ? (100 - gyroscopeAverageMovement) * 0.4 : 0;
            const combinedRiskScore = Math.round((clickRisk + gyroscopeRisk) * 100) / 100;

            const hasSuspiciousActivity = hasClickSuspicion || hasGyroscopeSuspicion || combinedRiskScore >= 40;
    ...
    static async getGyroscopeMonitoringStats(
        ...
        try {
            ...
            const gyroscopeEnabledRecords = data.filter(record => record.gyroscope_enabled);
            const gyroscopeEnabledGames = gyroscopeEnabledRecords.length;
            const gyroscopeEnabledPercentage = Math.round((gyroscopeEnabledGames / totalGames) * 10000) / 100;

            const totalMovementPercentage = gyroscopeEnabledRecords.reduce(
                (sum, record) => sum + (record.gyroscope_movement_percentage || 0), 0
            );
            const averageMovementPercentage = gyroscopeEnabledGames > 0
                ? Math.round((totalMovementPercentage / gyroscopeEnabledGames) * 100) / 100
                : 0;

            const suspiciousGyroscopeGames = data.filter(record => record.gyroscope_suspicious).length;
            const suspiciousGyroscopePercentage = gyroscopeEnabledGames > 0
                ? Math.round((suspiciousGyroscopeGames / gyroscopeEnabledGames) * 10000) / 100
                : 0;
            ...
            const totalChecks = gyroscopeEnabledRecords.reduce(
                (sum, record) => sum + (record.total_gyroscope_checks || 0), 0
            );
            const averageChecksPerGame = gyroscopeEnabledGames > 0
                ? Math.round((totalChecks / gyroscopeEnabledGames) * 100) / 100
                : 0;

            const totalSensitivity = gyroscopeEnabledRecords.reduce(
                (sum, record) => sum + (record.gyroscope_min_sensitivity || 0), 0
            );
            const averageSensitivity = gyroscopeEnabledGames > 0
            ...
            return {
                success: false,
                error: "Internal server error while fetching gyroscope monitoring statistics",
            };
        }
    }
