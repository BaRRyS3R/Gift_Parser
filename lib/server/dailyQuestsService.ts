...
export const serverDailyQuestsService = {
    ...
    async processGameQuestUpdates(
        ...
        // ЭТАП 7: НЕ начисляем попытки здесь - вернем информацию для gameService
        let attemptsAwarded = 0;

        if (isCompleted && !userQuest.is_completed) {
            console.log(`[DEBUG-QUEST-OPT] Quest completed! Will be awarded ${quest.reward_attempts} attempts by gameService (${Date.now() - startTime}ms)`);
            attemptsAwarded = quest.reward_attempts; // Только помечаем сколько нужно начислить
        }

        const totalTime = Date.now() - startTime;
        console.log(`[DEBUG-QUEST-OPT] Quest processing completed in ${totalTime}ms (no attempts awarded here)`);

        // ЭТАП 8: Возвращаем результат БЕЗ реального начисления attempts
        return [{
            questId: quest.id,
            completed: isCompleted,
            attemptsAwarded, // gameService увидит что нужно начислить
            previousProgress,
            newProgress,
            targetValue: quest.target_value,
        }];
