  ...
  async getUserRankings(telegramId: number, user?: any): Promise<UserRankings> {
    ...
    if (userData.total_games > 0) {
      const { count, error: overallError } = await supabaseServer
        ...
        .gt("total_games", 0)
        .or(
          `best_score.gt.${userData.best_score},and(best_score.eq.${userData.best_score},total_games.gt.${userData.total_games})`,
        );
    ...
    if (userData.survival_games > 0) {
      const { count, error: survivalError } = await supabaseServer
        ...
        .gt("survival_games", 0)
        .or(
          `survival_best_time.gt.${userData.survival_best_time},and(survival_best_time.eq.${userData.survival_best_time},survival_max_level.gt.${userData.survival_max_level})`,
        );
    ...
    if (userData.physics_games > 0) {
      const { count, error: physicsError } = await supabaseServer
        ...
        .gt("physics_games", 0)
        .or(
          `physics_best_score.gt.${userData.physics_best_score},and(physics_best_score.eq.${userData.physics_best_score},physics_best_time.gt.${userData.physics_best_time})`,
        );
    ...
    if (userData.rotation_games > 0) {
      const { count, error: rotationError } = await supabaseServer
        ...
        .gt("rotation_games", 0)
        .or(
          `rotation_best_time.gt.${userData.rotation_best_time},and(rotation_best_time.eq.${userData.rotation_best_time},rotation_max_level.gt.${userData.rotation_max_level})`,
        );
