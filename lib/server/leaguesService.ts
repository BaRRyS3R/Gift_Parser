...
export const serverLeaguesService = {
   ...
   */
  async getCompactLeagueData(
    userId: string,
    totalGames: number,
  ): Promise<CompactLeagueData | null> {
    try {
      const progressInfo = await leagueService.getUserLeagueProgress(
        userId,
        totalGames,
      );
      ...
      return {
        ...
        nextLeague: progressInfo.nextLeague
          ? this.sanitizeLeague(progressInfo.nextLeague)
          : null,
   ...
   */
  async getFullLeagueData(
    userId: string,
    totalGames: number,
  ): Promise<FullLeagueData> {
    try {
      const [
        progressInfo,
        allLeagues,
      ...
      leaderboardResults.forEach(({ leagueId, leaderboard }) => {
        if (leaderboard) {
          leaderboards[leagueId] = this.sanitizeLeagueLeaderboard(
            leaderboard,
            userId,
          );
        }
      ...
      return {
        ...
        userRewards: userRewards.map((reward) =>
          this.sanitizeUserReward(reward),
        ),
        leaderboards,
        neighbors: neighbors
          ? this.sanitizeLeagueNeighbors(neighbors, userId)
          : null,
   ...
   */
  sanitizeAllLeagueRewards(
    allRewards: Record<number, any[]>,
  ): Record<number, SafeLeagueReward[]> {
