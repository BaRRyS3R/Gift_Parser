...
export const serverLeagueService = {
  ...
  async getCompleteLeagueData(
    ...
    try {
      console.log(
        `Fetching complete league data for user: ${userId}, games: ${totalGames}`,
      );

      const [leagues, progressInfo, userRewards, neighbors, allLeagueRewards] =
        await Promise.all([
          this.getAllLeagues(),
      ...
      console.log(
        `Successfully fetched complete league data for user ${userId}:`,
        {
  ...
  async initializeUserLeague(
    ...
    try {
      ...
      await supabaseServer.from("user_leagues").insert({
        user_id: userId,
        league_id: league.id,
  ...
  async checkAndUpdateLeague(
    ...
    try {
      ...
      if (newLeague.name !== "bronze") {
        rewardResult = await this.claimLeagueRewardImproved(
          userId,
          newLeague.id,
          newTotalGames,
        );
