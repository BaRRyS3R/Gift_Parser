// src/lib/server/leagueServerService.ts - Server-side league management service
...
export const serverLeagueService = {
    ...
    /**
     * Get user league progress information
     */
    ...
    /**
     * Get complete league data for user
     */
    async getCompleteLeagueData(userId: string, totalGames: number): Promise<CompleteLeagueData> {
        try {
            console.log(`Fetching complete league data for user: ${userId}, games: ${totalGames}`);

            const [leagues, progressInfo, userRewards, neighbors, allLeagueRewards] = await Promise.all([
                this.getAllLeagues(),
            ...
            const leaderboards: Record<number, LeagueLeaderboard> = {};
            ...
            console.log(`Successfully fetched complete league data for user ${userId}:`, {
                leagues: leagues.length,
                hasProgressInfo: !!progressInfo,
                userRewards: userRewards.length,
                hasNeighbors: !!neighbors,
                leaderboards: Object.keys(leaderboards).length
            });
    ...
    /**
     * Improved league reward claiming logic
     */
    async claimLeagueRewardImproved(userId: string, leagueId: number, gamesCount: number): Promise<LeagueRewardResult> {
        try {
            ...
            // Check that user has reached this league
            if (gamesCount < league.min_games) {
                return { success: false, reason: 'insufficient_games' };
            }

            // Check if user already received reward for this league
            ...
            // Get users sorted by league achievement time
            const { data: usersInLeague, error: usersError } = await supabaseServer
                .from('user_leagues')
                .select(`
                    user_id,
            ...
            // Find current user position among league achievers
            ...
            // Check available rewards for this position
    ...
    /**
     * Check for missed rewards retroactively
     */
    async checkMissedRewards(userId: string): Promise<LeagueRewardResult[]> {
        try {
            ...
            // Check each achieved league for missed rewards
            for (const userLeague of userLeagueHistory) {
                ...
                // If reward was not received, try to give it
                if (!existingReward) {
    ...
    /**
     * Check and update league with missed rewards check
     */
    async checkAndUpdateLeague(userId: string, newTotalGames: number): Promise<{
        ...
        try {
            ...
            if (currentUserLeague && currentUserLeague.league_id === newLeague.id) {
                // Even if league didn't change, check for missed rewards
            ...
            if (newLeague.name !== 'bronze') {
                rewardResult = await this.claimLeagueRewardImproved(userId, newLeague.id, newTotalGames);
            }

            // Check for missed rewards from previous leagues
            const missedRewards = await this.checkMissedRewards(userId);
