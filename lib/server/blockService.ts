...
export const serverBlockService = {
    ...
    async checkVerificationAttempt(
        telegramId: number,
    ): Promise<{ attempt: VerificationAttempt | null; isExpired: boolean; inGracePeriod: boolean }> {
        try {
            const { data, error } = await supabaseServer
                .from("verification_attempts")
                .select("*")
            ...
            const inGracePeriod = data.permission_granted_at &&
 ...
 */
    async isWithinPermissionGracePeriod(permissionGrantedAt: string): Promise<boolean> {
    ...
    async markPermissionGranted(
        ...
        try {
            ...
            if (error) {
                ...
                console.log(`Permission granted marked for attempt ${attemptId}, restart required: ${requiresRestart}`);
            }
        } catch (error) {
            console.error("Error marking permission granted:", error);
        }
