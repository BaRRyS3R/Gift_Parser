// src/lib/redis.ts - Redis клиент для Upstash
...
export const REDIS_KEYS = {
  ...
  // Пользовательские данные (для будущего расширения)
  USER_PROFILE: (telegramId: number) => `user:${telegramId}:profile`,
  USER_ATTEMPTS: (telegramId: number) => `user:${telegramId}:attempts`,
  
  // Сезонные данные (для будущего расширения)
  SEASON_DATA: (seasonId: string) => `season:${seasonId}:data`,
} as const;
