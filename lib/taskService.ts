...
export const taskService = {
  ...
  async getUserTasks(userId: string): Promise<TaskWithCompletion[]> {
    try {
      ...
      // Объединяем данные
      const tasksWithCompletion: TaskWithCompletion[] = (tasks || []).map(
        (task) => ({
          ...task,
  ...
  async findUserReliably(
    ...
    try {
      console.log(
        `Searching for user: UUID=${userId}, telegramId=${telegramId}`,
      );
      ...
      if (telegramId) {
        console.log(
          `User not found by UUID ${userId}, trying telegramId ${telegramId}`,
        );

        const { data: userByTelegramId, error: telegramError } =
          await supabaseServer
            .from("users")
            .select("id, telegram_id, attempts_remaining")
        ...
        console.log(`Search by telegramId result:`, {
          userByTelegramId,
          telegramError,
        });
        ...
        if (userByTelegramId && !telegramError) {
          console.log(
            `User found by telegramId: ${telegramId}, UUID: ${userByTelegramId.id}`,
          );
      ...
      console.error(
        `User not found by UUID ${userId}${telegramId ? ` or telegramId ${telegramId}` : ""}`,
      );
  ...
  async completeTask(
    ...
    try {
      console.log(
        `Starting task completion for user ${userId}, task ${taskId}`,
      );
      ...
      console.log(
        `Processing task ${taskId} for user ${userId}, reward: ${task.attempts_reward}`,
      );

      // Надежный поиск пользователя
      const { user: existingUser, foundBy } = await this.findUserReliably(
        userId,
        telegramId,
      );
      ...
      if (!existingUser) {
        console.error(
          `User not found with ID: ${userId}${telegramId ? ` or telegramId: ${telegramId}` : ""}`,
        );
      ...
      console.log(
        `User found by ${foundBy}: ${actualUserId} with ${existingUser.attempts_remaining} attempts`,
      );

      // Выполняем операции атомарно
      const newAttemptsTotal =
        existingUser.attempts_remaining + task.attempts_reward;
      ...
      if (updateError) {
        console.error(
          "Error updating user attempts in users table:",
          updateError,
        );
      ...
      console.log(
        `Successfully updated attempts for user ${actualUserId}: ${existingUser.attempts_remaining} -> ${newAttemptsTotal}`,
      );
      ...
      if (insertError) {
        console.error(
          "Error completing task, rolling back attempts:",
          insertError,
        );
      ...
      console.log(
        `Task ${taskId} completed successfully for user ${actualUserId}, awarded ${task.attempts_reward} attempts`,
      );
  ...
  async verifyTask(
    ...
    try {
      switch (taskType) {
        ...
        case "telegram_chat":
          return await this.verifyTelegramMembership(
            userId,
            taskId,
            verificationData,
          );
        case "twitter_follow":
          return await this.verifyTwitterFollow(
            userId,
            taskId,
            verificationData,
          );
        case "twitter_repost":
          return await this.verifyTwitterRepost(
            userId,
            taskId,
            verificationData,
          );
        case "visit_website":
          return await this.verifyWebsiteVisit(
            userId,
            taskId,
            verificationData,
          );
        case "telegram_story":
          return await this.verifyTelegramStory(
            userId,
            taskId,
            verificationData,
          );
  ...
  async verifyTelegramMembership(
    ...
    try {
      ...
      console.log(
        `Verifying Telegram membership for user ${userId} in channel ${task.channel_id}`,
      );
  ...
  async verifyTwitterFollow(
    ...
    try {
      console.log(
        `Verifying Twitter follow for user ${userId}, task ${taskId}`,
      );
  ...
  async verifyTwitterRepost(
    ...
    try {
      console.log(
        `Verifying Twitter repost for user ${userId}, task ${taskId}`,
      );
      const isVerified = true; // В реальности нужно проверить через Twitter API
      ...
      return {
        success: true,
        message: isVerified
          ? "Twitter repost verified"
          : "Twitter repost not verified",
        verified: isVerified,
      };
    } catch (error) {
  ...
  async verifyTelegramStory(
    ...
    try {
      console.log(
        `Verifying Telegram story for user ${userId}, task ${taskId}`,
      );
      const isVerified = true; // В реальности нужно проверить через Telegram API
      ...
      return {
        success: true,
        message: isVerified
          ? "Telegram story verified"
          : "Telegram story not verified",
        verified: isVerified,
      };
    } catch (error) {
  ...
  async getUserTaskStats(userId: string): Promise<{
    ...
    try {
      ...
      const totalAttemptsEarned =
        completedTasks?.reduce(
          (sum, task) => sum + (task.attempts_awarded || 0),
          0,
        ) || 0;
