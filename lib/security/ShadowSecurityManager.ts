...
export class ShadowSecurityManager {
  ...
  private currentOrientationData: {
    alpha: number | null;
    beta: number | null;
    gamma: number | null;
  } | null = null;
  private lastOrientationData: {
    alpha: number | null;
    beta: number | null;
    gamma: number | null;
  } | null = null;
  ...
  constructor(
    ...
    const shouldEnableGyroscope =
      !isIOSDevice && gyroscopeConfig?.enabled !== false;
    ...
    this.gyroscopeConfig = {
      ...
      suspiciousMovementThreshold:
        ShadowSecurityManager.DEFAULT_GYROSCOPE_SUSPICIOUS_THRESHOLD,
    ...
    this.state = {
      ...
      gameStartTime,
      suspiciousThreshold:
        ShadowSecurityManager.DEFAULT_SUSPICIOUS_CLICK_THRESHOLD,
      clickRecords: [],
    ...
    if (isIOSDevice) {
      console.log(
        "Shadow Security: iOS device detected - gyroscope monitoring disabled for performance optimization",
        {
    ...
    if (this.gyroscopeConfig.enabled && !isIOSDevice) {
      ...
      console.log(
        "Shadow Security: Skipping gyroscope initialization for iOS device optimization",
      );
    }
  }
  ...
  private detectIOSDevice(): boolean {
    ...
    const hasIOSTouch =
      ...
      !/Chrome|Android/i.test(userAgent);
    ...
    const isIOSDevice =
      isIOSUserAgent ||
      isIOSPlatform ||
      (isIOSStandalone && hasIOSTouch) ||
      (hasIOSTouch && isIOSTouchPoints);

    if (isIOSDevice) {
      console.log(
        "Shadow Security: Device detection results - iOS device confirmed",
        {
  ...
  private async initializeGyroscopeMonitoring(): Promise<void> {
    try {
      ...
      this.state.gyroscopeMonitoring.permissionGranted =
        capability.permissionGranted;
      this.state.gyroscopeMonitoring.dataAvailable = capability.dataAvailable;

      if (!capability.isSupported) {
        this.state.gyroscopeMonitoring.errorReason =
          GyroscopeErrorReason.NOT_SUPPORTED;
      ...
      if (!capability.permissionGranted) {
        this.state.gyroscopeMonitoring.errorReason =
          GyroscopeErrorReason.PERMISSION_DENIED;
        this.state.gyroscopeMonitoring.enabled = false;
      ...
      if (!capability.dataAvailable) {
        this.state.gyroscopeMonitoring.errorReason =
          GyroscopeErrorReason.DATA_UNAVAILABLE;
        this.state.gyroscopeMonitoring.enabled = false;
      ...
      this.state.gyroscopeMonitoring.errorReason =
        GyroscopeErrorReason.INITIALIZATION_FAILED;
      this.state.gyroscopeMonitoring.enabled = false;
    }
  }
  ...
  private performGyroscopeMovementCheck(): void {
    ...
    if (this.state.gyroscopeMonitoring.lastCheckTime > 0) {
      const actualInterval =
        currentTime - this.state.gyroscopeMonitoring.lastCheckTime;
    ...
    const movementDetected = this.detectMovement(
      this.lastOrientationData,
      this.currentOrientationData,
    );
    ...
    const movementPercentage =
      this.state.gyroscopeMonitoring.movementRecords.length > 0
        ? (this.state.gyroscopeMonitoring.movementRecords.filter(
            (r) => r.movementDetected,
          ).length /
            this.state.gyroscopeMonitoring.movementRecords.length) *
          100
        : 0;
  }
  ...
  private generateGyroscopeResult(): GyroscopeMonitoringResult {
    ...
    const movementsDetected = movements.filter(
      (record) => record.movementDetected,
    ).length;
    const movementPercentage =
      totalChecks > 0 ? (movementsDetected / totalChecks) * 100 : 0;

    // Calculate if suspicious based on configurable threshold
    const isSuspicious =
      movementPercentage < this.gyroscopeConfig.suspiciousMovementThreshold;

    const checkIntervals = this.state.gyroscopeMonitoring.checkIntervals;
    const averageCheckInterval =
      checkIntervals.length > 0
        ? checkIntervals.reduce((sum, interval) => sum + interval, 0) /
          checkIntervals.length
        : 0;
   ...
   */
  public generateSuspiciousActivityData(
    telegramId: number,
    gameEndTime: number,
    ...
    if (
      ...
      console.log(
        "Shadow Security: iOS device optimization - skipping all suspicious activity tracking",
      );
    ...
    const suspiciousClicks = clickRecords.filter(
      (record) => record.isSuspicious,
    );
    ...
    const hasGyroscopeIssues =
      this.gyroscopeConfig.enabled &&
      !this.state.gyroscopeMonitoring.enabled &&
      this.state.gyroscopeMonitoring.errorReason !== null &&
      this.state.gyroscopeMonitoring.errorReason !==
        "ios_optimization_disabled";
    ...
    if (
      ...
      console.log(
        "Shadow Security: No suspicious activity or gyroscope issues detected - skipping submission",
      );
