            ...
            <div className="flex items-center justify-center space-x-2">
              ...
              <div className="text-center">
                ...
                <p className="text-white/80">
                  {t("leagues.notifications.levelUp.message", {
                    level: notification.level || 1,
                  })}
                </p>
              </div>
            </div>
        ...
        return (
          <div className="space-y-4">
            <div className="text-center space-y-3">
              <div
                className={`w-16 h-16 ${leagueColors.bg} rounded-full flex items-center justify-center border ${leagueColors.border} mx-auto`}
              >
                <LeagueIcon
                  className={`${leagueColors.text} animate-pulse-gentle`}
                  size={28}
                />
              </div>
              ...
              <div>
                ...
                <p className="text-white/90 text-lg">
                  {t("leagues.notifications.leaguePromotion.message", {
                    league: t(
                      `leagues.names.${notification.league.name}` as any,
                    ),
                  })}
                </p>
              </div>
              ...
              {notification.position && (
                <div
                  className={`inline-block px-3 py-1 rounded-full ${leagueColors.bg} border ${leagueColors.border}`}
                >
                  <span className={`text-sm font-bold ${leagueColors.text}`}>
                    {t("leagues.notifications.leaguePromotion.position", {
      ...
      case "reward_received":
        ...
        return (
          <div className="space-y-4">
            <div className="text-center space-y-3">
              <div
                className={`w-16 h-16 ${rewardColors.bg} rounded-full flex items-center justify-center border ${rewardColors.border} mx-auto`}
              >
                <Gift
                  className={`${rewardColors.text} animate-pulse-gentle`}
                  size={28}
                />
              </div>
              ...
              <div
                className={`inline-block px-3 py-1 rounded-full ${rewardColors.bg} border ${rewardColors.border}`}
              >
                <span className={`text-sm font-bold ${rewardColors.text}`}>
                  {t("leagues.notifications.rewardReceived.position", {
                    ...
                    league: t(
                      `leagues.names.${notification.reward.league}` as any,
                    ),
                  })}
                </span>
              </div>
