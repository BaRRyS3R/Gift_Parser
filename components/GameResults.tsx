...
export default function GameResults({
  ...
  const getDeathCauseIcon = (cause?: string) => {
    switch (cause) {
      ...
      default:
        return <Skull className="text-red-400" size={20} />;
    }
  };
  ...
  const getDeathCauseMessage = (cause?: string): string => {
    switch (cause) {
      case "miss":
        return "Failed to hit a white target";
      case "wrong_click":
        return "Clicked an inactive target";
      case "decoy_hit":
        return "Clicked a red trap";
      case "timeout":
        return "Time ran out";
      default:
  ...
  const getDifficultyIcon = (difficulty: GameDifficulty) => {
    switch (difficulty) {
      case GameDifficulty.LEGENDARY:
        return Award; // VETERAN
      case GameDifficulty.OMG:
        return Flame; // MANIAC
      case GameDifficulty.NIGHTMARE:
        return Skull; // DEMON
      case GameDifficulty.IMPOSSIBLE:
        return Crown; // GODLIKE
      case GameDifficulty.PRECISION:
        return Crosshair;
      default:
        return Target;
  ...
  const sarcasticComment = isPrecisionMode
    ? getPrecisionComment()
    : getStandardComment();
      ...
      <div className="w-full max-w-md space-y-8 animate-fade-in">
        <div className="text-center space-y-2">
          ...
          <div className="flex items-center justify-center space-x-2">
            <DifficultyIcon className={colors.primary} size={20} />
            <p className={`text-lg font-bpdots ${colors.secondary}`}>
              {getDifficultyDisplayName(result.difficulty)}{" "}
              {isPrecisionMode ? "Mode" : "Difficulty"}
            </p>
          </div>
          {isPrecisionMode && result.deathCause && (
            <div
              className={`flex items-center justify-center space-x-2 ${colors.background} border ${colors.border} rounded-lg p-2`}
            >
        ...
        {/* Comment Section */}
        <div
          className={`backdrop-blur-sm border rounded-xl p-4 ${colors.background} ${colors.border}`}
        >
          <div className="text-center">
            <div
              className={`text-xl font-bold font-bpdots ${rating.color} mb-2`}
            >
              {rating.text}
            </div>
          ...
          >
            {isSaving && (
              <div className="flex items-center justify-center space-x-3">
                <Spinner
                  color={isPrecisionMode ? "danger" : "default"}
                  size="sm"
                />
                <div className="text-center">
                  <span className={`font-bpdots text-sm ${colors.secondary}`}>
              ...
              <div className="text-center space-y-3">
                ...
                <div className="bg-red-500/10 border border-red-400/30 rounded-lg p-3">
                  <div className="text-xs font-bpdots text-red-300/80 text-center space-y-1">
                    ...
                    <div>
                      Your game was completed but results couldn&apos;t be saved
                    </div>
        ...
        {/* Results Section */}
        <div
          className={`backdrop-blur-sm border rounded-xl p-6 space-y-6 ${colors.background} ${colors.border}`}
        >
          ...
          {isPrecisionMode ? (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center space-y-1">
                  <div className={`text-xs font-bpdots ${colors.accent}`}>
                    SURVIVAL TIME
                  </div>
                  <div
                    className={`text-xl font-bold font-bpdots ${colors.primary}`}
                  >
                ...
                <div className="text-center space-y-1">
                  ...
                  <div
                    className={`text-xl font-bold font-bpdots text-orange-400`}
                  >
                ...
                <div className="text-center space-y-1">
                  ...
                  <div
                    className={`text-xl font-bold font-bpdots text-green-400`}
                  >
              ...
              {result.maxIntensityReached && result.maxIntensityReached > 1 && (
                <div className={`border-t ${colors.border} pt-4`}>
                  <div className="text-center space-y-2">
                    ...
                    <div
                      className={`text-lg font-bold font-bpdots text-orange-400`}
                    >
                      Level {result.maxIntensityReached}:{" "}
                      {getPrecisionLevelDescription(result.maxIntensityReached)}
                    </div>
                    <div className="w-full h-2 bg-red-900/20 rounded-full overflow-hidden border border-red-400/30">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, ((result.maxIntensityReached || 1) / 15) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
              ...
              {result.averageReactionTime > 0 && (
                <div className={`border-t ${colors.border} pt-4`}>
                  <div className="text-center space-y-1">
                    ...
                    <div
                      className={`text-lg font-bold font-bpdots ${getReactionTimeColor()}`}
                    >
