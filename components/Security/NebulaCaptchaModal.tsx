// src/components/Security/NebulaCaptchaModal.tsx - Adapted for Nebula Security System

"use client";

import React, { useState, useEffect, useRef } from "react";
import { Shield, RefreshCw, Clock, AlertTriangle, CheckCircle2 } from "lucide-react";
...
const NebulaCaptchaModal: React.FC<NebulaCaptchaModalProps> = ({
    ...
    useEffect(() => {
        if (isOpen && !state.challenge && !state.attemptMade) {
            generateCaptcha();
        }
    ...
    const handleSubmit = async () => {
        if (!state.challenge || !state.userInput.trim() || state.isValidating || state.attemptMade) {
            return;
        }
                        ...
                        {state.isLoading ? (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-gray-800 border border-gray-600 rounded-lg p-4 text-center">
                                    ...
                                    <p className="text-gray-400 text-xs">Enter the result of this calculation</p>
                                </div>
                                ...
                                <div className="flex items-center justify-center space-x-2 text-sm">
                                    ...
                                    <span
                                        className={`font-bold ${state.timeRemaining < 5000 ? "text-red-400" : "text-orange-400"
                                            }`}
                                    >
                                ...
                                <div>
                                    <input
                                        ...
                                        disabled={state.isValidating || state.timeRemaining === 0 || state.attemptMade}
                                        maxLength={10}
                ...
                {!state.isSuccess && (
                    <div className="mt-6 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                        <p className="text-yellow-300 text-xs text-center">
                            Security verification required due to low trust score. Your account will be temporarily
