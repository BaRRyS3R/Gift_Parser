...
const PrizeItem: React.FC<{ prize: Prize; index: number }> = ({
  prize,
  index,
}) => {
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10">
      <div className="flex items-center space-x-3">
        ...
        <span className={`font-bold ${positionColor}`}>#{prize.position}</span>
      </div>
...
const TournamentStats: React.FC<{ tournament: Tournament; mode: string }> = ({
  tournament,
  mode,
}) => {
  const t = useT();
  ...
  const relevantStats: StatItem[] = [
    {
      key: "duration",
      label: t("tournaments.details.duration"),
      type: "duration",
    },
    {
      key: "start_time",
      label: t("tournaments.stats.startDate"),
      type: "date",
    },
  ...
  const formatStatValue = (stat: StatItem, tournament: Tournament): string => {
    switch (stat.type) {
      ...
      case "date":
        const date = new Date(
          tournament[stat.key as keyof Tournament] as string,
        );
...
export default function TournamentDetail({
  ...
  const handleJoinTournament = useCallback(() => {
    if (
      ...
      console.error(
        "Invalid tournament mode or status:",
        tournament.mode,
        tournament.status,
      );
    }
          ...
          >
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-16 h-16 rounded-xl ${colors.background} border ${colors.border} flex items-center justify-center`}
                  >
                ...
                {tournament.status === "active" && timeLeft && (
                  <div className="text-right">
                    ...
                    <div className={`font-mono text-xl ${colors.primary}`}>
                      {timeLeft}
                    </div>
                  </div>
                )}
                ...
                {tournament.status === "upcoming" && timeLeft && (
                  <div className="text-right">
                    <div className="text-white/60 text-xs">
                      {t("tournaments.details.startsIn")}
                    </div>
                    <div className={`font-mono text-xl ${colors.primary}`}>
                      {timeLeft}
                    </div>
            ...
            <CardBody className="pt-2">
              ...
              <TournamentStats mode={tournament.mode} tournament={tournament} />
            </CardBody>
