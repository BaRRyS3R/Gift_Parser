...
export default function TournamentButton({
  ...
  return (
    ...
    >
      ...
      {isActive && (
        <div
          className={`absolute inset-0 ${colors.glow} opacity-20 animate-pulse`}
        />
      )}
      ...
      {/* Hover effect */}
      <div
        className={`absolute -inset-1 bg-gradient-to-r from-${colors.primary.replace("text-", "").replace("-400", "-500")}/20 via-transparent to-${colors.primary.replace("text-", "").replace("-400", "-500")}/20 rounded-xl blur opacity-0 group-hover:opacity-100 transition duration-300`}
      />
