// src/components/DailyQuestModal/DailyQuestModal.tsx - Оптимизировано
...
export default function DailyQuestModal({
  ...
  const modalFetchedRef = useRef(false); // Флаг для модального окна

  // Load quest data when modal opens - только если данные еще не загружены
  useEffect(() => {
    if (isOpen && !quest && !modalFetchedRef.current) {
      modalFetchedRef.current = true;
      fetchDailyQuest();
    }

    // Сбрасываем флаг при закрытии модального окна
    if (!isOpen) {
      modalFetchedRef.current = false;
    }
  }, [isOpen]); // ✅ ИСПРАВЛЕНО: убраны quest и fetchDailyQuest из зависимостей
  ...
  const handleRetry = () => {
    clearError();
    modalFetchedRef.current = false; // Сбрасываем флаг для повторной загрузки
              ...
              questContent && (
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-center space-x-2">
                    ...
                    <span
                      className={`text-sm font-mono uppercase ${getGameModeColor()}`}
                    >
                      {getGameModeName()}
                    </span>
                  </div>
