...
const SponsorBanner: React.FC<SponsorBannerProps> = ({ tournament }) => {
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      setTimeDisplay(
        diff > 0 ? formatTimeRemaining(diff) : t("tournament.ended"),
      );
    };
          ...
          <div className="space-y-1">
            <div className="text-xl font-bold text-white">
              {leaderboard.length > 0
                ? Math.max(...leaderboard.map((e) => e.survival_score))
                : 0}
            </div>
            <div className="text-xs text-white/60 uppercase tracking-wider font-medium">
...
const UserPositionComponent: React.FC<UserPositionComponentProps> = ({
  leaderboard,
  tournament,
}) => {
  const { user } = useUser();
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          >
            {userEntry.rank <= 3 ? (
              ...
              <span
                className={`text-lg font-bold ${isWinner ? "text-yellow-400" : "text-white"}`}
              >
                #{userEntry.rank}
              </span>
            )}
          ...
          <div>
            <div className="flex items-center space-x-2">
              <span
                className={`font-bold ${isWinner ? "text-yellow-400" : "text-white"}`}
              >
            ...
            <div className="flex items-center space-x-4 text-sm text-white/60 mt-1">
              <div className="flex items-center space-x-1">
                ...
                <span>
                  {userEntry.survival_score} {t("tournament.points")}
                </span>
              </div>
              <div className="flex items-center space-x-1">
                <Activity size={12} />
                <span>
                  {userEntry.games_played || 1} {t("tournament.gamesPlayed")}
                </span>
              </div>
              <div className="flex items-center space-x-1">
                <Clock size={12} />
                <span>
                  {formatTournamentSurvivalTime(userEntry.survival_time)}
                </span>
              </div>
            </div>
            {prize && (
        ...
        <div className="text-right">
          ...
          <div className="text-xs text-white/60">
            {t("tournament.outOf")} {leaderboard.length}
          </div>
        </div>
      </div>
    </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center space-x-3 text-xs text-white/60 mt-1">
                <div className="flex items-center space-x-1">
                  <Activity size={10} />
                  <span>
                    {participant.games_played || 1} {t("tournament.gamesShort")}
                  </span>
                </div>
                <div className="flex items-center space-x-1">
                  <Clock size={10} />
...
// Модальные окна участников, призов и правил
        ...
        <ModalHeader className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-medium text-white">
              {t("tournament.allParticipants")}
            </h2>
            <p className="text-white/60 text-sm">
              {participants.length} {t("tournament.totalParticipants")}
            </p>
          </div>
        </ModalHeader>
...
const PrizesModal: React.FC<PrizesModalProps> = ({
  ...
  const getRankIcon = (position: number) => {
    switch (position) {
      ...
      default:
        return <Trophy className="text-white/60" size={20} />;
    }
  };
    ...
    >
      <ModalContent>
        <ModalHeader className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-medium text-white">
              {t("tournament.tournamentPrizes")}
            </h2>
            <p className="text-white/60 text-sm">
              {prizes.length} {t("tournament.prizePositions")}
            </p>
          </div>
        </ModalHeader>
          ...
          {prizes.map((prize, index) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="text-white/80 font-medium mt-1">{prize}</div>
              </div>
            </div>
          ))}
        </ModalBody>
    ...
    >
      <ModalContent>
        <ModalHeader className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-medium text-white">
              {t("tournament.kingOfTheHillRules")}
            </h2>
            <p className="text-white/60 text-sm">
              {t("tournament.competitionFormat")}
            </p>
          </div>
        </ModalHeader>
            ...
            <div className="flex items-center space-x-3 mb-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-yellow-400">
                  {t("tournament.conceptTitle")}
                </h3>
                <p className="text-yellow-300/80 text-sm">
                  {t("tournament.conceptSubtitle")}
                </p>
              </div>
            </div>
...
const ActiveTournamentSection: React.FC<ActiveTournamentSectionProps> = ({
  ...
  const hasAttemptsRemaining =
    user?.attempts_remaining && user.attempts_remaining > 0;
      ...
      <div className="space-y-4 px-4 py-6">
        ...
        {!hasAttemptsRemaining && (
          <div className="bg-white/5 border border-white/20 rounded-lg p-4 text-center">
            <p className="text-white/60 text-sm">
              {t("tournament.attemptsUsed")}
            </p>
            <p className="text-white/40 text-xs mt-1">
              {t("tournament.waitForReset")}
            </p>
          </div>
        )}
      </div>
...
const UpcomingTournamentItem: React.FC<UpcomingTournamentItemProps> = ({
  tournament,
}) => {
  const t = useT();
  const [timeDisplay, setTimeDisplay] = useState<string>("");
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      setTimeDisplay(
        diff > 0 ? formatTimeRemaining(diff) : t("tournament.starting"),
      );
    };
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm text-white/60">
                ...
                <div className="flex items-center space-x-2">
                  <Trophy size={14} />
                  <span>
                    {tournament.prizes.length} {t("tournament.prizesShort")}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-white tracking-wide">
              {t("tournament.upcomingTournaments")}
            </h2>
            <p className="text-white/40 text-sm uppercase tracking-[0.2em]">
              {tournaments.length} {t("tournament.tournaments")}
            </p>
          </div>
        </div>
      </div>
...
const CompletedTournaments: React.FC<CompletedTournamentsProps> = ({
  tournaments,
}) => {
  const t = useT();
  const [loadedWinners, setLoadedWinners] = useState<
    Record<string, TournamentLeaderboardEntry[]>
  >({});
  const [expandedTournaments, setExpandedTournaments] = useState<Set<string>>(
    new Set(),
  );

  const loadWinners = async (tournamentId: string, prizeCount: number) => {
    ...
    try {
      ...
      setLoadedWinners((prev) => ({ ...prev, [tournamentId]: [] }));
    }
  };

  const handleToggleExpand = (tournamentId: string, prizeCount: number) => {
    setExpandedTournaments((prev) => {
      ...
      if (newSet.has(tournamentId)) {
        ...
        loadWinners(tournamentId, prizeCount);
      }
      return newSet;
    });
  };
    ...
    <div className="space-y-6">
      <div className="px-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-white/70 tracking-wide">
                ...
                <div className="w-full h-64 bg-gradient-to-br from-gray-800/60 to-gray-900/80 relative">
                  ...
                  <div className="relative h-full flex flex-col justify-between p-6">
                    ...
                    <div className="space-y-3">
                      {topWinner && (
                        <div className="flex items-center space-x-3">
                          <Crown className="text-yellow-400/80" size={20} />
                          <div>
                      ...
                      <div className="flex items-center justify-between text-sm text-white/60">
                        <div className="flex items-center space-x-2">
                          ...
                          <span>
                            ...
                            {t("tournament.participantsShort")}
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Trophy size={14} />
                  ...
                  <div className="relative h-full flex flex-col justify-between p-6">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-3">
                        {topWinner && (
                          <div className="flex items-center space-x-3">
                            <Crown className="text-yellow-400/80" size={18} />
                            <div>
                              <div className="text-lg font-bold text-white">
                      ...
                      {tournament.sponsor_channel_url && (
                        <div className="flex justify-end">
                          <button
                            onClick={handleSponsorClick}
                            className="flex items-center space-x-2 px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/40 rounded-lg transition-all duration-300 backdrop-blur-sm hover:scale-105 active:scale-95"
                          >
                                  ...
                                  <div className="text-right">
                                    <div className="text-sm font-bold text-white/80">
                                      <div className="flex items-center space-x-1">
                                        ...
                                        />
                                        <span>
                                          {winner.survival_score}{" "}
                                          {t("tournament.pointsShort")}
                                        </span>
                                      </div>
                                    </div>
                                  </div>
...
export default function TournamentsPage() {
      ...
      try {
        ...
        if (tournamentsData.active.length > 0) {
          ...
          const lbRes = await fetch(
            `/api/tournament/leaderboard?tournamentId=${activeTournament.id}`,
            {
              headers: {
                Authorization: "Bearer " + (localStorage.getItem("jwt") || ""),
              },
        ...
        <div className="text-center space-y-6">
          ...
          <div>
            <p className="text-white text-lg font-medium">
              {t("tournament.loadingTournaments")}
            </p>
            <p className="text-white/60 text-sm">
              {t("tournament.fetchingData")}
            </p>
          </div>
        </div>
      </div>
  ...
  const hasUpcomingTournaments = tournaments.upcoming.length > 0;
        ...
        <div className="text-center space-y-8 max-w-md mx-auto px-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white mb-3">
              {t("tournament.noTournamentsAvailable")}
            </h1>
  ...
  return (
    <div className="min-h-screen bg-black text-white safe-area-inset-bottom safe-area-inset">
      ...
      {hasUpcomingTournaments && (
        <div
          className={`${hasActiveTournament ? "py-6 border-t border-white/10" : "py-6"}`}
        >
      ...
      {hasCompletedTournaments && (
        <div
          className={`${hasActiveTournament || hasUpcomingTournaments ? "py-6 border-t border-white/10" : "py-6"}`}
        >
          <CompletedTournaments tournaments={tournaments.completed} />
        </div>
      )}
    </div>
  );
