...
const CompletedTournamentCard: React.FC<CompletedTournamentCardProps> = ({
  ...
  useEffect(() => {
    if (isExpanded && winners.length === 0) {
      const loadWinners = async () => {
        ...
        try {
          const tournamentWinners = await onLoadWinners(
            tournament.id,
            tournament.prizes.length,
          );
  ...
  const getRankIcon = (position: number) => {
    switch (position) {
      ...
      default:
        return (
          <span className="text-white/50 text-xs font-medium">#{position}</span>
        );
    }
  };
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="bg-white/5 border border-white/15 rounded-lg p-4">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <span className="text-sm font-bold text-white/70 uppercase tracking-wider">
                  {t("tournament.champions")} ({tournament.prizes.length}{" "}
  ...
  useEffect(() => {
    const loadTournaments = async () => {
      try {
        ...
        const tournamentsResponse = await makeAuthenticatedRequest(
          "/api/tournament/list",
        );
        ...
        if (!tournamentsResult.success) {
          throw new Error(
            tournamentsResult.error || "Failed to load tournaments",
          );
        }
        ...
        if (tournamentsData.active.length > 0) {
          ...
          console.log(
            "Loading leaderboard for active tournament:",
            activeTournament.id,
          );

          const leaderboardResponse = await makeAuthenticatedRequest(
        ...
        setError(
          err instanceof Error
            ? err.message
            : t("tournament.errorLoadingTournaments"),
        );
      } finally {
        setIsLoading(false);
      }
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-white/40 text-sm uppercase tracking-[0.2em]">
                  {tournaments.completed.length} {t("tournament.tournaments")}
                </p>
              </div>
            </div>
          </div>
