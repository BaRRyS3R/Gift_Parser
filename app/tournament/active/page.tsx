...
export default function ActiveTournamentPage() {
    ...
    const loadTournamentData = useCallback(async () => {
        try {
            ...
            if (!activeTournament) {
                setError("Нет активного турнира");
                setTimeout(() => {
            ...
            const [tournamentLeaderboard, userTournamentResult] = await Promise.all([
                tournamentService.getTournamentLeaderboard(activeTournament.id, 50),
                user?.id ? tournamentService.getUserTournamentResult(activeTournament.id, user.id) : null
            ]);
    ...
    const handleStartTournament = useCallback(async () => {
        if (!tournament || isTransitioning || !hasAttemptsRemaining()) return;

        setIsTransitioning(true);
        setTimeout(() => {
            router.push("/tournament/play");
        }, 600);
    }, [tournament, isTransitioning, router]);
    ...
    const renderLeaderboardEntry = (entry: TournamentLeaderboardEntry, position: number) => {
        const isWinner = position <= (tournament?.prizes.length || 0);
        const prize = isWinner ? tournament?.prizes[position - 1] : null;
        ...
        return (
            <div className="min-h-screen bg-black flex items-center justify-center">
                <div className="text-center space-y-4">
                    ...
                    <p className="text-white/60">{error || "Tournament not found"}</p>
                    <p className="text-white/40 text-sm">{t("tournament.redirectingToTournament")}</p>
                </div>
            </div>
        );
    }
