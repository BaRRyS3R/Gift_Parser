// src/app/tournaments/details/page.tsx - Static route implementation without dynamic segments
...
function TournamentDetailsContent() {
  ...
  // Redirect to tournaments list if no ID is provided
  useEffect(() => {
    if (!tournamentId) {
      console.warn("No tournament ID provided, redirecting to tournaments list");
      router.replace("/tournaments");
      return;
    }
  }, [tournamentId, router]);

  // Configure Telegram WebApp back button functionality
  useEffect(() => {
  ...
  // Handle cases where no tournament ID is provided
  if (!tournamentId) {
    return null;
  }

  // Display loading state while tournament data is being fetched
  ...
  // Display error state when tournament loading fails or tournament is not found
    ...
    <div className="min-h-screen bg-black text-white safe-area-inset-bottom">
      <div className="px-6 py-8">
        ...
        <div className={`rounded-xl border backdrop-blur-sm p-6 mb-6 ${colors.bg} ${colors.border}`}>
          ...
          {/* User Status and Action Buttons */}
        ...
        {activeTab === "overview" ? (
          ...
          leaderboard ? (
            <TournamentLeaderboard
              ...
              totalParticipants={leaderboard.total_participants || 0}
              stats={{
                total_participants: leaderboard.stats?.total_participants || 0,
                total_games_played: leaderboard.stats?.total_games_played || 0,
                average_score: leaderboard.stats?.average_score || 0,
                highest_score: leaderboard.stats?.highest_score || 0,
                games_per_day: leaderboard.stats?.games_per_day || [],
              }}
              isLoading={isLoading}
