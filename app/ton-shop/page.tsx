...
function TONShopContent() {
    ...
    useEffect(() => {
        if (!initData) {
            setError("Missing authentication data. Please access this page through the main app.");
            setIsLoading(false);
            return;
        }
        ...
        if (orderState.orderId && orderState.isPending) {
            ...
            const timeout = setTimeout(() => {
                ...
                setOrderState(prev => ({
                    ...prev,
                    isPending: false,
                    error: "Order monitoring timeout. Please check your transaction status manually.",
                }));
            }, 5 * 60 * 1000);
    ...
    const loadTONShopData = async () => {
        try {
            ...
            const response = await fetch(`/api/ton/products?initData=${encodeURIComponent(initData!)}`);
                    ...
                    <Card className="bg-white/10 border border-white/20 mb-6">
                        <CardBody className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h3 className="font-bold text-white">Welcome, {userInfo.firstName}!</h3>
                                    <p className="text-white/70 text-sm">Current attempts: {userInfo.attemptsRemaining}</p>
                                </div>
                                <Chip className="bg-white/20 text-white border border-white/30" size="sm">
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-white/70 text-sm">
                                        {wallet ? 'Connected' : 'Connect your TON wallet to make purchases'}
                                    </p>
                                </div>
                            </div>
                            {!wallet && (
                        ...
                        <CardBody className="p-4">
                            {orderState.isCreating && (
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white/70 text-sm">Preparing your purchase</p>
                                    </div>
                                </div>
                            )}
                            ...
                            {orderState.isPending && (
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-white">Payment Processing...</h4>
                                        <p className="text-white/70 text-sm">
                            ...
                            {orderState.isCompleted && (
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-white">Payment Successful!</h4>
                                            <p className="text-white/70 text-sm">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-white/70 text-sm">{orderState.error}</p>
                                        </div>
                                    </div>
                                    <Button
                                        className="bg-white/20 text-white border border-white/40"
                ...
                <Card className="bg-white/5 border border-white/10">
                    <CardBody className="p-6 text-center">
                        ...
                        <p className="text-white/70 text-sm mb-4">
                            • Payments are processed automatically within 5 minutes<br />
                            • You can safely close this page after payment<br />
                            • Attempts will be visible in the main app after processing<br />
                            • You will receive a notification when the payment is complete
                        </p>
...
function ProductCard({ product, onSelect, disabled, isSelected }: ProductCardProps) {
    const getBadge = () => {
        switch (product.productType) {
            case 'attempts_5':
                return { text: 'Popular', color: 'bg-blue-600/20 text-blue-300 border-blue-600/30' };
            case 'attempts_10':
                return { text: 'Best Value', color: 'bg-green-600/20 text-green-300 border-green-600/30' };
            case 'attempts_100':
                return { text: 'Ultimate', color: 'bg-purple-600/20 text-purple-300 border-purple-600/30' };
            default:
                return null;
        }
    };
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-white font-bold text-xl">{product.priceTON} TON</span>
                        </div>
