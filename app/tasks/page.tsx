...
export default function TasksPage() {
    ...
    useEffect(() => {
        const loadTasks = async () => {
            ...
            try {
                ...
                setTaskState(prev => ({
                    ...prev,
                    error: error instanceof Error ? error.message : t("tasks.failedToLoad")
                }));
            } finally {
                setTaskState(prev => ({ ...prev, isLoading: false }));
            }
    ...
    const showSuccessNotification = (task: TaskWithCompletion, attemptsAwarded: number) => {
        ...
        const plural = attemptsAwarded === 1 ? t("tasks.rewards.attempt") : t("tasks.rewards.attempts");
    ...
    const getTaskBadge = (task: TaskWithCompletion) => {
        if (task.is_completed) {
            return { text: "Completed", textKey: "tasks.status.completed", color: "success" };
        }
            ...
            <div className="max-w-2xl mx-auto space-y-4">
                {tasks.length === 0 ? (
                    <div className="text-center py-12 bg-white/10 backdrop-blur-xl border border-white/30 rounded-lg">
                        ...
                        <p className="text-sm mt-1 text-white/60">{t("tasks.noTasksDescription")}</p>
                    </div>
                ) : (
                    tasks.map((task) => (
                        <TaskCard
                    ...
                    <div className="flex-1">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center space-x-2 mb-1">
                                    ...
                                    {badge && (
                                        <Chip
                                            className={`
                        ${badge.color === 'success' ? 'bg-green-500/20 text-green-300 border border-green-400/30' :
                                                    badge.color === 'warning' ? 'bg-yellow-500/20 text-yellow-300 border border-yellow-400/30' :
                      ...
                      `}
                                ...
                                <p className="text-white/70 text-sm">
                                    {task.description || t(`tasks.types.${task.type.replace('_', '')}Desc`)}
                                </p>
                                {!task.is_completed && (
