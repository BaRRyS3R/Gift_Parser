// src/app/game/page.tsx - Game Mode Selection Page with Accessibility Compliance
  ...
  const renderModeCard = (mode: GameMode) => {
    ...
    return (
      <button
        ...
        className={`
          w-full text-left backdrop-blur-sm overflow-hidden border-2 rounded-2xl font-bpdots 
          transition-all duration-500 group focus:outline-none focus:ring-2 focus:ring-white/50
          disabled:cursor-not-allowed disabled:opacity-50
          ${
            isSelected
              ? `${mode.color.background} ${mode.color.border} scale-105`
              : "bg-white/5 border-white/20 hover:bg-white/10 hover:border-white/40 hover:scale-105 focus:bg-white/10 focus:border-white/40"
          }
        `}
      ...
      >
        <div className="p-6">
            ...
            <div
              ...
              ${
                isSelected
                  ...
                  : "bg-white/10 group-hover:bg-white/20 group-focus:bg-white/20"
              }
            `}
            >
              <Icon
                size={24}
                className={isSelected ? mode.color.primary : "text-white"}
              />
            </div>
              ...
              <p
                className={`text-sm ${
                  isSelected
                    ...
                    : "text-white/60 group-hover:text-white/80 group-focus:text-white/80"
                }`}
              >
                {mode.description}
              </p>
            ...
            <div className="text-right">
              <div
                className={`text-xs uppercase tracking-wider mb-1 ${
                  isSelected ? mode.color.accent : "text-white/40"
                }`}
              >
                {mode.difficulty}
              </div>
              <div
                className={`text-xs ${
                  isSelected ? mode.color.secondary : "text-white/60"
                }`}
              >
                {mode.duration}
              </div>
              ...
              <div key={index} className="flex items-center space-x-2">
                <div
                  className={`w-1 h-1 rounded-full ${
                    isSelected ? mode.color.primary : "bg-white/40"
                  }`}
                ></div>
                <span
                  className={`text-xs ${
                    isSelected ? mode.color.accent : "text-white/70"
                  }`}
                >
                  {feature}
                </span>
          ...
          <div
            className={`
              w-full py-3 px-4 rounded-lg font-bpdots text-sm font-bold transition-all duration-300 text-center
              ${
                isSelected
                  ? `${mode.color.background} ${mode.color.primary} ${mode.color.border} border`
                  : "bg-white/10 text-white/80 group-hover:bg-white/15 group-focus:bg-white/15 border border-white/20"
              }
            `}
          >
