...
export default function NebulaPage(): JSX.Element {
  ...
  const openVerificationModal = useCallback(
    (verificationType?: VerificationType, attemptId?: string) => {
      const vType = verificationType || pageState.verificationType;
      const aId = attemptId || pageState.attemptId;

      if (!vType || !aId) {
        console.error(
          "Cannot open verification modal: missing verification type or attempt ID",
          {
    ...
    try {
      ...
      if (result.verification) {
        setPageState((prev) => ({
          ...prev,
          phase:
            result.verification!.type === "captcha"
              ? "ready_to_verify"
              : "device_checking",
  ...
  const handlePermissionGranted = useCallback(() => {
    ...
    setTimeout(
      () =>
        openVerificationModal(
          pageState.verificationType || undefined,
          pageState.attemptId || undefined,
        ),
      1000,
    );
   ...
   */
  const blockForUnsupportedDevice = useCallback(
    async (type: VerificationType) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to block user: ${response.status} ${response.statusText}`,
          );
        }
  ...
  const blockForPermissionTimeout = useCallback(async () => {
    if (!pageState.attemptId || !pageState.verificationType) {
      console.error(
        "Missing attempt ID or verification type for permission timeout blocking",
      );
      router.push("/blocked");
  ...
  const handleCloseModal = useCallback(() => {
    ...
    setPageState((prev) => ({
      ...prev,
      phase: pageState.deviceCapability.permissionGranted
        ? "ready_to_verify"
        : "permission_required",
    }));
  ...
  useEffect(() => {
    if (
      pageState.phase === "ready_to_verify" &&
      pageState.verificationType &&
      pageState.attemptId
    ) {
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-gray-900 border border-gray-700 rounded-xl p-6 text-center">
          ...
          <p className="text-gray-400 text-sm mb-4">
            Проверяем, поддерживает ли ваше устройство требуемый тип
            аутентификации...
          </p>
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-gray-900 border border-red-500/30 rounded-xl p-6 text-center">
          ...
          <p className="text-red-200 text-xs mb-6">{pageState.error}</p>
        ...
        <div className="max-w-md w-full bg-gray-900 border border-gray-700 rounded-xl p-6">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400 text-sm">
              Для продолжения необходимо предоставить разрешение на
              использование{" "}
              {pageState.verificationType === "biometric"
            ...
            <div className="flex items-start space-x-2">
              ...
              <div>
                ...
                <p className="text-red-200 text-xs">
                  Если вы не предоставите разрешение в течение{" "}
                  {formatTime(pageState.permissionTimer)}, ваш аккаунт будет
          ...
          {pageState.deviceCapability.permissionRequested && (
            <p className="text-gray-400 text-xs text-center mt-3">
              Ожидаем предоставления разрешения... Проверка каждые{" "}
              {PERMISSION_CHECK_INTERVAL_MS / 1000} секунд
            </p>
          )}
        </div>
      </div>
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-gray-900 border border-gray-700 rounded-xl p-6">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400 text-sm">
              Устройство поддерживает требуемый тип аутентификации. Начинаем
              верификацию...
            </p>
          </div>
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-gray-900 border border-green-500/30 rounded-xl p-6 text-center">
          ...
          <p className="text-gray-400 text-xs">
            {t("nebula.verification.success.redirecting")}
          </p>
        </div>
      </div>
    );
