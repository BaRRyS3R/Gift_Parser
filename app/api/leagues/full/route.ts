...
export async function GET(
  request: NextRequest,
): Promise<NextResponse<LeagueFullAPIResponse>> {
  try {
    ...
    console.log(
      `Fetching full league data for user: ${userId}, games: ${user.total_games}`,
    );

    // Get complete league data
    const leagueData = await serverLeaguesService.getFullLeagueData(
      userId,
      user.total_games,
    );
