...
export async function GET(request: NextRequest) {
    try {
        ...
        const determineCurrentLeague = (totalGames: number): LeagueData | null => {
            ...
            return allLeagues.find(l => l.name === 'bronze') || allLeagues[0] || null;
        };
        ...
        // Find next league
        const nextLeague = allLeagues.find(league =>
            league.min_games > user.total_games
        );
        ...
        if (nextLeague) {
            ...
            if (currentLeagueRange > 0) {
                progressPercent = Math.min(100, (gamesInCurrentLeague / currentLeagueRange) * 100);
            }
        } else {
