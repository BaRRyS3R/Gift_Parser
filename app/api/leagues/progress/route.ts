...
export async function GET(
  request: NextRequest,
): Promise<NextResponse<LeagueProgressAPIResponse>> {
  try {
    ...
    const { data: user, error: userError } = await import(
      "@/lib/supabase_server"
    ).then((module) =>
    ...
    console.log(
      `Fetching league progress for user: ${userId}, games: ${user.total_games}`,
    );

    // Get compact league data
    const leagueData = await serverLeaguesService.getCompactLeagueData(
      userId,
      user.total_games,
    );
