...
export async function GET(
  request: NextRequest,
): Promise<NextResponse<LeaguesResponse>> {
  try {
    ...
    console.log(
      `Fetching complete league data for user ${telegramId} with ${totalGames} games`,
    );

    // Get complete league data
    const leagueData = await serverLeagueService.getCompleteLeagueData(
      userId,
      totalGames,
    );
