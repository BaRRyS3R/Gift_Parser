...
export async function GET(request: NextRequest) {
    try {
        ...
        const currentLevel = Math.min(Math.floor(user.total_games / GAMES_PER_LEVEL) + 1, MAX_LEVEL);
        ...
        if (allLeagues) {
            const nextLeagueData = allLeagues.find(league =>
                league.min_games > totalGames
            );
            ...
            if (nextLeagueData) {
                ...
                const currentLeagueData = allLeagues.find(league =>
                    league.min_games <= totalGames
                );

                if (currentLeagueData) {
                    const gamesInCurrentLeague = totalGames - currentLeagueData.min_games;
                    const gamesNeededForNextLeague = nextLeagueData.min_games - currentLeagueData.min_games;
                    progressPercent = (gamesInCurrentLeague / gamesNeededForNextLeague) * 100;
                }
            } else {
