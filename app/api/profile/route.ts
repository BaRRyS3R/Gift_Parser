...
import { serverProfileService, type ProfileResponse } from '@/lib/server/profileService';
...
export async function GET(request: NextRequest): Promise<NextResponse<ProfileAPIResponse>> {
    try {
        ...
        const profileData = await serverProfileService.getProfileData(telegramIdNumber);
 ...
 */
export async function PATCH(request: NextRequest): Promise<NextResponse<ProfileAPIResponse>> {
    try {
        // Extract user info from middleware headers
        const telegramId = request.headers.get('X-Telegram-ID');
        const userId = request.headers.get('X-User-ID');
        ...
        // Update profile
        const updatedProfile = await serverProfileService.updateProfile(telegramIdNumber, updates);
