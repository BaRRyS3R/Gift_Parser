...
export const POST = withAuth(async (request) => {
  try {
    ...
    console.log(
      `User ${user.telegramId} attempting to verify task ${taskId} of type ${verificationType}`,
    );
    ...
    if (!verificationResult.success) {
      console.warn(
        `Task verification failed for user ${user.telegramId}, task ${taskId}: ${verificationResult.error}`,
      );
    ...
    console.log(
      `Task ${taskId} verification result for user ${user.telegramId}: ${verificationResult.verified ? "verified" : "not verified"}`,
    );
