...
export const POST = withAuth(async (request) => {
    try {
        ...
        if (typeof scoreChange !== 'number') {
            return NextResponse.json({ success: false, error: 'Invalid scoreChange' }, { status: 400 });
        }
        ...
        return NextResponse.json({ success: false, error: 'Failed to update trust score' }, { status: 500 });
    }
});