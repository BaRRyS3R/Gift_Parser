...
export const GET = withAuth(async (request) => {
    try {
        ...
        const now = new Date();
        const blockedUntil = userData.blocked_until ? new Date(userData.blocked_until) : null;
        ...
        return NextResponse.json({ success: false, error: 'Failed to check security status' }, { status: 500 });
    }
});