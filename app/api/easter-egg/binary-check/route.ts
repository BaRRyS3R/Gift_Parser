...
export async function POST(
    ...
    try {
        ...
        console.log(`Binary check attempt: "${binaryString}" vs secret: "${SECRET_BINARY}"`);

        // Check if the binary string matches our secret
        const isCorrect = binaryString === SECRET_BINARY;

        if (isCorrect) {
            console.log(`Correct binary sequence entered by user ${telegramId || 'anonymous'}`);
            ...
            console.log(`Incorrect binary sequence: "${binaryString}" by user ${telegramId || 'anonymous'}`);
