...
const EASTER_EGG_REWARDS: Record<EasterEggType, {
...
export async function POST(
    ...
    try {
        ...
        const { data: existingAchievement, error: checkError } = await supabaseServer
        ...
        const { data: result, error: transactionError } = await supabaseServer
            .rpc('award_easter_egg_achievement', {
                p_user_id: userId,
        ...
        console.log(`🎉 Easter Egg "${easterEggType}" found by user ${telegramIdNumber}! Awarded ${reward.attempts} attempts.`);
