...
export async function POST(
  request: NextRequest,
): Promise<NextResponse<WinnersResponse>> {
  try {
    ...
    console.log(
      `Fetching winners for tournament ${tournamentId} (prizes: ${prizeCount}) by user: ${telegramId}`,
    );

    // Get tournament winners
    const winners = await serverTournamentService.getTournamentWinners(
      tournamentId,
      prizeCount,
    );
