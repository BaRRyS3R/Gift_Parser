...
export async function POST(
  request: NextRequest,
): Promise<NextResponse<TournamentSaveAPIResponse>> {
  try {
    ...
    console.log(
      `Processing tournament save request for user ${telegramIdNumber}:`,
      {
        tournamentId,
    ...
    if (!saveResult.success) {
      ...
      if (
        saveResult.error?.includes("not active") ||
        saveResult.error?.includes("not currently running")
      ) {
        return NextResponse.json(
          {
            success: false,
    ...
    console.log(
      `Successfully processed tournament save for user ${telegramIdNumber}:`,
      {
        tournamentId,
