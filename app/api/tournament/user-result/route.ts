...
import { serverTournamentService, type TournamentResult } from '@/lib/server/tournamentService';
 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<UserResultResponse>> {
    try {
        ...
        console.log(`Fetching user result for tournament ${tournamentId} by user: ${userId}`);

        // Get user tournament result
        const userResult = await serverTournamentService.getUserTournamentResult(tournamentId, userId);
