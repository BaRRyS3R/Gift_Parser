...
export const GET = withAuth(async (request) => {
    try {
        ...
        return NextResponse.json({ isActive: true, activeTournament, timeRemaining });
    } catch (error) {
        console.error('Error fetching active tournament:', error);
        return NextResponse.json({ success: false, error: 'Failed to fetch active tournament' }, { status: 500 });
    }
}); 