...
export async function GET(
    ...
    try {
        ...
        console.log("Tournaments API called with params:", { tournament, telegramId });
            ...
            try {
                ...
                if (!tournamentData) {
                    ...
                    if (parts.length >= 1) {
                        ...
                        const allTournaments = await serverTournamentService.getAllTournaments();
                ...
                if (telegramId) {
                    const telegramIdNumber = parseInt(telegramId);
                    if (!isNaN(telegramIdNumber)) {
                        console.log("Getting user position for telegram ID:", telegramIdNumber);
                        userPosition = await serverTournamentService.getUserTournamentPosition(
                            tournamentData.id,
                            telegramIdNumber,
                        );
