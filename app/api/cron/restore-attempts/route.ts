...
function getLocalizedMessage(
    ...
    console.log(`[CRON_LOCALIZATION] Language_code length: ${languageCode?.length || 'null'}`);

    // Безопасная нормализация языкового кода
    const normalizedLanguageCode = languageCode?.toString().toLowerCase().trim();
    console.log(`[CRON_LOCALIZATION] Normalized language_code: "${normalizedLanguageCode}"`);
    ...
    console.log(`[CRON_LOCALIZATION] Use Russian: ${useRussian}, Selected language: ${userLanguage}`);

    // Получаем шаблон сообщения
    const messageTemplate = NOTIFICATION_TRANSLATIONS[userLanguage].notifications.restored.fullMessage;
    ...
    console.log(`[CRON_LOCALIZATION] Final message preview: ${finalMessage.substring(0, 50)}...`);
...
export async function POST(request: NextRequest): Promise<NextResponse<CronResponse>> {
    ...
    try {
        ...
        const blockedByUsers = notificationResults.filter(result => result.blocked_by_user).length;
        const rateLimitedFailures = notificationResults.filter(result =>
            !result.success && result.error?.includes("rate limit")
        ).length;
        const otherFailures = notificationResults.filter(result =>
            !result.success && !result.blocked_by_user && !result.error?.includes("rate limit")
        ).length;

        const executionTime = Date.now() - startTime;

        console.log(`[CRON] Restoration completed: ${usersToProcess.length} users processed, ${totalRestored} attempts restored`);
        console.log(`[CRON] Notifications: ${successfulNotifications} sent, ${blockedByUsers} blocked, ${rateLimitedFailures} rate limited, ${otherFailures} other failures`);
...
async function getUsersWithExpiredResetTime() {
    ...
    const { data, error } = await supabaseServer
        .from("users")
        .select("id, telegram_id, first_name, language_code, attempts_remaining, attempts_reset_at")
...
async function sendRestorationNotifications(restorationResults: RestorationResult[]) {
    ...
    const usersToNotify = restorationResults.filter(result => result.attempts_restored > 0);

    console.log(`[CRON] Sending notifications to ${usersToNotify.length} users in batches of ${TELEGRAM_RATE_LIMIT}`);
    ...
    for (let i = 0; i < usersToNotify.length; i += TELEGRAM_RATE_LIMIT) {
        ...
        console.log(`[CRON] Processing notification batch ${batchNumber}/${totalBatches} (${batch.length} users)`);
        ...
        const batchPromises = batch.map(async (result) => {
            try {
                ...
                console.error(`[CRON] Unexpected error sending notification to user ${result.telegram_id}:`, error);
                return {
                    telegram_id: result.telegram_id,
                    success: false,
                    error: error instanceof Error ? error.message : "Unknown error",
    ...
    const skippedUsers = restorationResults.filter(result => result.attempts_restored === 0);
    ...
    const blockedByUsers = notificationResults.filter(r => r.blocked_by_user).length;
    const otherFailures = notificationResults.filter(r => !r.success && !r.blocked_by_user).length;

    console.log(`[CRON] Notification summary: ${successfulNotifications} sent, ${blockedByUsers} blocked by users, ${otherFailures} other failures`);
...
async function sendTelegramNotificationWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const result = await sendTelegramNotification(telegramId, firstName, languageCode, attemptsRestored);

            if (result.success) {
                if (attempt > 1) {
                    console.log(`[CRON] Notification sent successfully to user ${telegramId} on attempt ${attempt}`);
                }
                return { success: true };
            } else if (result.blocked_by_user) {
                // Пользователь заблокировал бота - повторные попытки бесполезны
                console.log(`[CRON] User ${telegramId} has blocked the bot - skipping further attempts`);
                return { success: false, blocked_by_user: true, error: "User blocked bot" };
                ...
                console.log(`[CRON] Rate limited for user ${telegramId}, waiting ${delayMs}ms before retry ${attempt}/${maxRetries}`);
                await new Promise(resolve => setTimeout(resolve, delayMs));
                continue;
            } else {
                // Другие ошибки - логируем и пробуем еще раз
                console.warn(`[CRON] Notification attempt ${attempt}/${maxRetries} failed for user ${telegramId}: ${result.error}`);
            ...
            console.error(`[CRON] Network error on attempt ${attempt}/${maxRetries} for user ${telegramId}:`, error);
            if (attempt < maxRetries) {
...
async function sendTelegramNotification(
    ...
    try {
        ...
        console.error(`[CRON_NOTIFICATION] Network error for user ${telegramId}:`, error);
        return {
            success: false,
