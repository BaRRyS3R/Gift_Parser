...
export async function POST(request: NextRequest): Promise<NextResponse<CronResponse>> {
    ...
    try {
        ...
        if (!apiKey || apiKey !== CRON_CONFIG.CRON_API_KEY) {
            console.warn("[TON_MONITOR] Unauthorized attempt to access ton-payments-monitor endpoint");
...
async function fetchRecentTransactions(): Promise<GetBlockTransaction[]> {
    const lookbackTimestamp = Math.floor((Date.now() - CRON_CONFIG.LOOKBACK_HOURS * 60 * 60 * 1000) / 1000);
    ...
    try {
        ...
        console.log(`[TON_MONITOR] GetBlock API returned ${data.transactions?.length || 0} transactions`);

        return data.transactions || [];
    } catch (error) {
        console.error("[TON_MONITOR] Error fetching transactions from GetBlock:", error);
 ...
 */
async function processTransactions(transactions: GetBlockTransaction[]): Promise<ProcessedTransaction[]> {
    const results: ProcessedTransaction[] = [];

    for (const transaction of transactions) {
        try {
            // Проверяем, не обработана ли уже эта транзакция
            const existingTransaction = await checkTransactionExists(transaction.hash);
            if (existingTransaction) {
                console.log(`[TON_MONITOR] Transaction ${transaction.hash} already processed, skipping`);
                continue;
            }
            ...
            console.error(`[TON_MONITOR] Error processing transaction ${transaction.hash}:`, error);
        }
    }
 ...
 */
async function processTransaction(transaction: GetBlockTransaction): Promise<ProcessedTransaction | null> {
    ...
    if (!parseResult.isValid) {
        ...
        console.warn(`[TON_MONITOR] Incorrect payload in transaction ${transaction.hash}: ${parseResult.error}`);

        await saveIncorrectTransaction(transaction, senderWallet, amountNanotons, payload, parseResult.error || 'Unknown parsing error');
    ...
    if (!telegramId || !productType || !uniqueId) {
        console.warn(`[TON_MONITOR] Incomplete parsed data for transaction ${transaction.hash}`);
    ...
    if (amountNanotons !== expectedAmount) {
        console.warn(`[TON_MONITOR] Amount mismatch for transaction ${transaction.hash}:`, {
            received: formatTONAmount(amountNanotons),
            expected: formatTONAmount(expectedAmount),
        });

        await saveIncorrectTransaction(transaction, senderWallet, amountNanotons, payload, `Amount mismatch: received ${formatTONAmount(amountNanotons)} TON, expected ${formatTONAmount(expectedAmount)} TON`);
    ...
    if (!user) {
        console.warn(`[TON_MONITOR] User not found for transaction ${transaction.hash}: telegram_id ${telegramId}`);
    ...
    try {
        ...
        console.log(`[TON_MONITOR] Successfully processed transaction ${transaction.hash}:`, {
            telegramId,
            productType,
        ...
        console.error(`[TON_MONITOR] Error saving transaction ${transaction.hash}:`, error);
        throw error;
    }
}
...
async function saveIncorrectTransaction(
    ...
    if (error) {
        console.error(`[TON_MONITOR] Error saving incorrect transaction ${transaction.hash}:`, error);
        throw error;
    }
}
...
async function saveSuccessfulTransaction(
    ...
    if (error) {
        console.error(`[TON_MONITOR] Error saving successful transaction ${transaction.hash}:`, error);
        throw error;
    }
}
...
async function creditAttemptsToUser(telegramId: number, attempts: number): Promise<void> {
    ...
    console.log(`[TON_MONITOR] Credited ${attempts} attempts to user ${telegramId}. New total: ${newAttemptsCount}`);
}
 ...
 */
async function sendSuccessNotifications(processResults: ProcessedTransaction[]): Promise<Array<{ success: boolean; telegram_id?: number; error?: string }>> {
    const successfulTransactions = processResults.filter(result => result.status === 'processed' && result.telegram_id);

    if (successfulTransactions.length === 0) {
        return [];
    }

    console.log(`[TON_MONITOR] Sending success notifications to ${successfulTransactions.length} users`);

    const notificationPromises = successfulTransactions.map(async (transaction) => {
        if (!transaction.telegram_id) return { success: false, error: 'No telegram_id' };
...
async function sendTONSuccessNotification(
    ...
    try {
        ...
        if (result.ok) {
            ...
            console.error(`[TON_MONITOR] Failed to send notification to user ${telegramId}:`, result);
            return false;
        }
    } catch (error) {
