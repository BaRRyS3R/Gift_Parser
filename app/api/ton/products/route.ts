 ...
 */
export async function GET(request: NextRequest): Promise<NextResponse<TONProductsResponse>> {
    try {
        ...
        const productTypes: ProductType[] = ['attempts_1', 'attempts_5', 'attempts_10', 'attempts_100'];
        const products = productTypes.map(productType => getTONProductInfo(productType));

        console.log(`[TON_PRODUCTS] Catalog requested by user ${user.telegram_id} (${user.first_name})`);
        ...
        console.error("Error fetching TON products:", error);
