...
export const POST = withAuth(async (request) => {
    try {
        ...
        if (userData.attempts_remaining <= 0) {
            return NextResponse.json({ success: false, error: 'No attempts remaining' }, { status: 400 });
        }
        // Списываем попытку
        ...
        return NextResponse.json({ success: false, error: 'Failed to consume attempt' }, { status: 500 });
    }
});