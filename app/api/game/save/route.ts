...
import { serverGameService, type GameSaveResponse, type GameResult } from '@/lib/server/gameService';
 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<GameSaveAPIResponse>> {
    try {
        ...
        const saveResult = await serverGameService.saveGameResult(telegramIdNumber, gameResult as GameResult);
        ...
        console.log(`Successfully processed game save for user ${telegramIdNumber}:`, {
            leagueChanged: saveResult.leagueChanged,
            levelChanged: saveResult.levelChanged,
            hasReward: !!saveResult.reward,
            hasMissedRewards: !!(saveResult.missedRewards && saveResult.missedRewards.length > 0)
        });
