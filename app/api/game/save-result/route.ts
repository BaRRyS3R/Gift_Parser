...
export const POST = withAuthAndRateLimit(async (request) => {
  try {
    ...
    // Save game result
    const saveResult = await userService.saveGameResult(
      user.telegramId,
      gameResult,
    );
...
function validateGameResult(gameResult: any): {
  valid: boolean;
  error?: string;
} {
  // Basic validation
  ...
  switch (gameResult.mode) {
    case GameMode.REACTION:
      if (
        typeof gameResult.reactionTime !== "number" ||
        gameResult.reactionTime < 0
      ) {
    ...
    case GameMode.PHYSICS:
      ...
      if (
        typeof gameResult.totalHits !== "number" ||
        gameResult.totalHits < 0
      ) {
