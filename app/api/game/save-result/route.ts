  ...
  try {
    ...
    if (gameResult.mode === GameMode.REACTION) {
      ...
      if (!gameResult.missed && gameResult.reactionTime > 0) {
        ...
        const newAverage =
          totalReactionGames > 0
            ? (currentAverage * totalReactionGames + gameResult.reactionTime) /
      ...
      updates.physics_total_hits =
        (user.physics_total_hits || 0) + gameResult.totalHits;
      ...
      updates.rotation_total_hits =
        (user.rotation_total_hits || 0) + gameResult.correctHits;
    }
    ...
    if (isCompetitiveMode) {
      try {
        const leagueCheckResult = await leagueService.checkAndUpdateLeague(
          userId,
          newTotalGames,
        );
