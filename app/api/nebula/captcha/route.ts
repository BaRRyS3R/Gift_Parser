// src/app/api/nebula/captcha/route.ts - Nebula Captcha Verification API
 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<CaptchaResponse>> {
    try {
        ...
        if (action === 'generate') {
            ...
            console.log(`Generated captcha for user ${telegramIdNumber}: ${captcha.challenge} = ${captcha.correctAnswer}`);
            ...
            const { userAnswer, challenge, completedInTime } = body as ValidateCaptchaRequest;
            ...
            if (isCorrect && completedInTime) {
                console.log(`Captcha verification successful for user ${telegramIdNumber}`);

                // Restore trust score
                const restoreResult = await serverBlockService.handleVerificationSuccess(telegramIdNumber, 'captcha');
                ...
                console.log(`Captcha verification failed for user ${telegramIdNumber}. Correct: ${isCorrect}, In time: ${completedInTime}`);
