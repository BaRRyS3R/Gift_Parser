...
function calculateGracePeriodRemaining(permissionGrantedAt: string): number {
    ...
    const minutesPassed = (currentTime.getTime() - grantTime.getTime()) / (1000 * 60);
...
function isWithinPermissionGracePeriod(permissionGrantedAt: string): boolean {
    ...
    const grantTime = new Date(permissionGrantedAt);
    const currentTime = new Date();
    const minutesDiff = (currentTime.getTime() - grantTime.getTime()) / (1000 * 60);
...
export async function GET(
    ...
    try {
        ...
        const { attempt, isExpired } = await serverBlockService.checkVerificationAttempt(telegramIdNumber);

        if (attempt) {
            // Check if permission was granted and requires app restart
            if (attempt.permissionGrantedAt && attempt.requiresAppRestart) {
                const inGracePeriod = isWithinPermissionGracePeriod(attempt.permissionGrantedAt);

                if (inGracePeriod) {
                    console.log(`User ${telegramIdNumber} in permission grace period, allowing continuation`);

                    const gracePeriodRemaining = calculateGracePeriodRemaining(attempt.permissionGrantedAt);
                    ...
                    console.log(`User ${telegramIdNumber} grace period expired, treating as abandoned`);
                    // Grace period expired, treat as abandoned
                    const abandonResult = await serverBlockService.handleAbandonedVerification(attempt);
            ...
            if (isExpired) {
                console.log(`User ${telegramIdNumber} has abandoned verification attempt, blocking`);
            ...
            if (!isExpired) {
                console.log(`User ${telegramIdNumber} has active verification attempt, redirecting to continue`);
