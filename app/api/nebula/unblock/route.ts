 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<UnblockResponse>> {
    try {
        ...
        if (!blockInfo || !blockInfo.isActive || blockInfo.timeRemainingSeconds <= 0) {
            ...
            console.log(`User ${telegramIdNumber} is not currently blocked or block has expired`);
            ...
            console.log(`User ${telegramIdNumber} is still blocked for ${blockInfo.timeRemainingSeconds} seconds`);
 ...
 */
export async function GET(request: NextRequest): Promise<NextResponse<UnblockResponse>> {
    try {
        // Extract user info from middleware headers
        const telegramId = request.headers.get('X-Telegram-ID');
        const userId = request.headers.get('X-User-ID');
