// src/app/api/nebula/biometric/route.ts - Updated with new permission-related block reasons
...
interface BiometricRequest {
    ...
    unavailable?: boolean; // New: biometric unavailable on device
 ...
 * Enhanced biometric verification with new permission-related blocking
 */
export async function POST(
    ...
    try {
        ...
        if (unavailable || !deviceSupported) {
            ...
            const blockReason = unavailable ? "biometric_unavailable" : "device_unsupported_biometric";
            ...
            if (blockResult.success) {
                return NextResponse.json({
                    ...
                    blockReason: unavailable
                        ? "Biometric authentication unavailable on device"
                        : "Device does not support biometric authentication",
                    blockDuration: "2 days",
                });
            } else {
                console.error(
                    "Failed to block user for biometric unavailability:",
                    blockResult.error,
                );
        ...
        if (permissionDenied) {
            ...
            if (blockResult.success) {
                return NextResponse.json({
                    ...
                    verified: false,
                    blocked: true,
                    blockReason: "Biometric authentication permission denied",
                    blockDuration: "2 days",
                });
            } else {
                console.error(
