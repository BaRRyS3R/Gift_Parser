// src/app/api/nebula/biometric/route.ts - Nebula Biometric Verification API
 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<BiometricResponse>> {
    try {
        ...
        const { success: biometricSuccess, completedInTime, deviceSupported, token } = body;

        console.log(`Biometric verification attempt for user ${telegramIdNumber}:`, {
        ...
        if (!deviceSupported) {
            console.log(`Biometric not supported for user ${telegramIdNumber} - blocking for 2 days`);
        ...
        if (biometricSuccess && completedInTime) {
            console.log(`Biometric verification successful for user ${telegramIdNumber}`);

            // Restore trust score
            const restoreResult = await serverBlockService.handleVerificationSuccess(telegramIdNumber, 'biometric');
            ...
            console.log(`Biometric verification failed for user ${telegramIdNumber}: ${failureReason}`);
