...
export function useGame(makeAuthenticatedRequest: (endpoint: string, options?: RequestInit) => Promise<Response>) {
     ...
     */
    const getActiveTournament = useCallback(async (): Promise<TournamentStatus> => {
        setState(prev => ({ ...prev, isLoading: true, error: null }));
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to fetch active tournament';
     ...
     */
    const getAllTournaments = useCallback(async (): Promise<TournamentListResponse> => {
        setState(prev => ({ ...prev, isLoading: true, error: null }));
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to fetch tournament list';
     ...
     */
    const getTournamentLeaderboard = useCallback(async (
        tournamentId: string,
        limit: number = 100
        ...
        try {
            ...
            const response = await makeAuthenticatedRequest('/api/tournament/leaderboard', {
                method: 'POST',
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to fetch tournament leaderboard';
    ...
    const getTournamentWinners = useCallback(async (
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to fetch tournament winners';
     ...
     */
    const getUserTournamentResult = useCallback(async (
        tournamentId: string
        ...
        try {
            ...
            const response = await makeAuthenticatedRequest('/api/tournament/user-result', {
                method: 'POST',
            ...
            if (!result.success) {
                throw new Error(result.error || 'Failed to fetch user tournament result');
            }
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to fetch user tournament result';
