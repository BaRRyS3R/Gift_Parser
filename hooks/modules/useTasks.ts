...
export function useTasks(
   ...
   */
  const startTaskWithTimer = useCallback(
    async (task: TaskWithStatus): Promise<boolean> => {
      setState((prev) => ({
        ...prev,
        ...
        setState((prev) => {
          const updatedTasks = prev.tasks.map((t) =>
            t.task_id === task.task_id
              ? { ...t, user_status: TaskStatus.STARTED }
              : t,
          );
   ...
   */
  const verifyTaskAfterTimer = useCallback(
    async (task: TaskWithStatus): Promise<void> => {
  ...
  const isTaskLoading = useCallback(
    (taskId: string, action: "start" | "verify" | "claim"): boolean => {
      switch (action) {
        case "start":
  ...
  return {
    ...
    inProgressTasks: state.tasks.filter(
      (task) => task.user_status === TaskStatus.STARTED,
    ),
  };
}