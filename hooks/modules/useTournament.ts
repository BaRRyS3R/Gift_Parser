 ...
 */
export function useTournament(makeAuthenticatedRequest: (endpoint: string, options?: RequestInit) => Promise<Response>) {
    ...
    const saveTournamentResult = useCallback(async (
        ...
        const attemptSave = async (): Promise<TournamentSaveResponse> => {
            try {
                console.log(`Attempting to save tournament result (attempt ${attemptCount}/${maxRetries}):`, {
                    tournamentId,
     ...
     */
    const saveTournamentResultWithRetry = useCallback(async (
        tournamentId: string,
        gameResult: SurvivalGameResult,
        ...
        const attemptSaveWithCallback = async (): Promise<TournamentSaveResponse> => {
     ...
     */
    const isValidTournamentResult = useCallback((gameResult: any): gameResult is SurvivalGameResult => {
     ...
     */
    const isValidTournamentId = useCallback((tournamentId: any): tournamentId is string => {
