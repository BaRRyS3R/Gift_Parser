...
export function useTournaments(
    ...
    const leaderboardCache = useRef<Map<string, {
     ...
     */
    const getTournamentStatus = useCallback((tournament: Tournament): TournamentTimeInfo => {
    ...
    const fetchTournamentDetail = useCallback(
        async (tournamentId: string): Promise<Tournament | null> => {
            setState((prev) => ({ ...prev, isDetailLoading: true, detailError: null }));
            ...
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : "Failed to fetch tournament detail";
    ...
    const fetchTournaments = useCallback(
        async (force: boolean = false): Promise<TournamentsData | null> => {
            // Check cache first
            if (!force && tournamentsCache.current &&
                isCacheValid(tournamentsCache.current.timestamp, CACHE_DURATION)) {
                setState(prev => ({ ...prev, tournaments: tournamentsCache.current!.data }));
    ...
    const fetchTournamentLeaderboard = useCallback(
        async (tournamentQuery: string, force: boolean = false): Promise<TournamentLeaderboardData | null> => {
            // Check cache first
            const cacheKey = tournamentQuery;
            const cached = leaderboardCache.current.get(cacheKey);

            if (!force && cached && isCacheValid(cached.timestamp, LEADERBOARD_CACHE_DURATION)) {
                setState(prev => ({
                    ...prev,
     ...
     */
    const isUserParticipating = useCallback((tournamentId: string): boolean => {
        return state.userPosition !== null && state.selectedTournament?.id === tournamentId;
     ...
     */
    const refreshLeaderboard = useCallback((tournamentQuery: string) => {
