...
export function useTONPayments() {
     ...
     */
    const fetchTONProducts = useCallback(async (initData: string): Promise<{
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to fetch TON products';
     ...
     * Создание TON заказа
     */
    const createTONOrder = useCallback(async (
        initData: string,
        productType: ProductType
        ...
        try {
            ...
            console.log('[TON_PAYMENTS] Order created successfully:', data.order.id);

            return data.order;
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Failed to create TON order';
            setState(prev => ({
                ...prev,
                isCreatingOrder: false,
                error: errorMessage
     ...
     */
    const sendTONTransaction = useCallback(async (order: CreateTONOrderResponse['order']): Promise<boolean> => {
        if (!wallet) {
     ...
     */
    const checkOrderStatus = useCallback(async (orderId: string): Promise<TONOrderStatusResponse['order'] | null> => {
        try {
     ...
     */
    const startOrderMonitoring = useCallback((orderId: string) => {
        ...
        setTimeout(() => {
            if (monitoringRef.current) {
                ...
                setState(prev => ({
                    ...prev,
                    isMonitoringPayment: false,
                    error: 'Payment monitoring timeout. Please check your transaction manually.'
                }));
            }
        }, 5 * 60 * 1000);
    }, [checkOrderStatus]);
     ...
     */
    const processTONPurchase = useCallback(async (
        initData: string,
        productType: ProductType
    ): Promise<boolean> => {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'TON purchase failed';
            setState(prev => ({ ...prev, error: errorMessage }));

            return false;
        }
    }, [wallet, openWalletModal, createTONOrder, sendTONTransaction, startOrderMonitoring]);
