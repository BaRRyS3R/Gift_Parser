...
export function useLeagues(
   ...
   */
  const fetchCompactLeagues = useCallback(
    async (forceRefresh = false): Promise<CompactLeagueData | null> => {
      // Check cache validity
      const now = Date.now();
      const isCacheValid =
        state.compactData &&
        !forceRefresh &&
        now - state.compactLastUpdate < COMPACT_CACHE_DURATION;
   ...
   */
  const fetchFullLeagues = useCallback(
    async (forceRefresh = false): Promise<FullLeagueData | null> => {
      // Check cache validity
      const now = Date.now();
      const isCacheValid =
        state.fullData &&
        !forceRefresh &&
        now - state.fullLastUpdate < FULL_CACHE_DURATION;
  ...
  const getCachedFullLeagues = useCallback((): FullLeagueData | null => {
    const now = Date.now();
    const isCacheValid =
      state.fullData && now - state.fullLastUpdate < FULL_CACHE_DURATION;
  ...
  const getCurrentLeague = useCallback((): SafeLeague | null => {
    return (
      state.compactData?.currentLeague ||
      state.fullData?.progress.currentLeague ||
      null
    );
  ...
  const getCurrentLevel = useCallback((): number | null => {
    return (
      state.compactData?.currentLevel ||
      state.fullData?.progress.currentLevel ||
      null
    );
  }, [state.compactData, state.fullData]);
  ...
  const hasRewards = useCallback((): boolean => {
    return !!(
      state.fullData?.userRewards && state.fullData.userRewards.length > 0
    );
  }, [state.fullData]);
  ...
  return {
    ...
    hasCompactData,
    hasFullData,
    hasValidCompactCache:
      state.compactData &&
      Date.now() - state.compactLastUpdate < COMPACT_CACHE_DURATION,
    hasValidFullCache:
      state.fullData && Date.now() - state.fullLastUpdate < FULL_CACHE_DURATION,
  };
}
