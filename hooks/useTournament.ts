...
interface TournamentHookReturn {
    ...
    loadLeaderboard: (tournamentId: string, limit?: number) => Promise<TournamentLeaderboardEntry[]>;
    ...
    getTournamentWinners: (tournamentId: string, prizeCount: number) => Promise<TournamentLeaderboardEntry[]>;
...
export function useTournament(): TournamentHookReturn {
    ...
    const loadActiveTournament = useCallback(async (): Promise<Tournament | null> => {
        if (!isUserAuthenticated()) {
            handleError(new Error('User not authenticated'), 'loadActiveTournament');
            return null;
        }
    ...
    const loadTournamentStatus = useCallback(async (): Promise<TournamentStatus> => {
        if (!isUserAuthenticated()) {
            const errorStatus: TournamentStatus = { isActive: false, activeTournament: null };
            handleError(new Error('User not authenticated'), 'loadTournamentStatus');
            return errorStatus;
        }
    ...
    const loadLeaderboard = useCallback(async (
        ...
        try {
            const leaderboard = await authService.getTournamentLeaderboard(tournamentId, limit);
    ...
    const loadUserResult = useCallback(async (tournamentId: string): Promise<TournamentResult | null> => {
        if (!isUserAuthenticated()) {
    ...
    const loadAllTournaments = useCallback(async (): Promise<TournamentListResponse> => {
        if (!isUserAuthenticated()) {
            const emptyResponse: TournamentListResponse = { active: [], upcoming: [], completed: [] };
    ...
    const saveTournamentResult = useCallback(async (
        ...
        try {
            const result = await authService.saveTournamentResult(tournamentId, gameResult);
    ...
    const getTournamentWinners = useCallback(async (
        tournamentId: string,
        prizeCount: number
    ): Promise<TournamentLeaderboardEntry[]> => {
        if (!isUserAuthenticated()) {
            handleError(new Error('User not authenticated'), 'getTournamentWinners');
            return [];
        }
        ...
        try {
            const winners = await authService.getTournamentWinners(tournamentId, prizeCount);
