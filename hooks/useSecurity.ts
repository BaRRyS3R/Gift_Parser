...
export function useSecurity(): SecurityHookReturn {
    ...
    const checkSecurity = useCallback(async (): Promise<SecurityCheckResult> => {
        if (!isAuthenticated || isCheckingRef.current) {
            throw new Error('Cannot check security: user not authenticated or check in progress');
        }
    ...
    useEffect(() => {
        if (isAuthenticated && isSecurityCheckNeeded() && !showCaptcha && !showBiometric) {
    ...
    }, [isAuthenticated, isSecurityCheckNeeded, showCaptcha, showBiometric, triggerSecurityCheck]);
