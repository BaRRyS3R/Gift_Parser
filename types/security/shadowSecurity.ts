 ...
 */
export function hasAnySuspiciousActivity(data: SuspiciousActivityData): boolean {
 ...
 */
export function calculateCombinedRiskScore(data: SuspiciousActivityData): number {
    let riskScore = 0;

    // Click-based risk (40% weight)
    if (data.totalClicks > 0) {
        const clickSuspiciousPercentage = (data.suspiciousClicksCount / data.totalClicks) * 100;
    ...
    const gameDurationMinutes = (data.gameEndTime - data.gameStartTime) / (1000 * 60);
