...
// Утилитарные типы
export type TournamentCreateData = Omit<Tournament, 'id' | 'status' | 'created_at' | 'updated_at'>;
export type TournamentUpdateData = Partial<Pick<Tournament, 'name' | 'description' | 'start_date' | 'end_date' | 'prizes' | 'max_participants' | 'is_active'>>;

// Маппинг игровых режимов к турнирным
export const GAME_MODE_TO_TOURNAMENT_MODE: Record<GameMode, TournamentGameMode | null> = {
...
// Обратный маппинг
export const TOURNAMENT_MODE_TO_GAME_MODE: Record<TournamentGameMode, GameMode> = {
...
// Функции-хелперы
export function getTournamentStatusFromDates(startDate: string, endDate: string): TournamentStatus {
    const now = new Date();
