...
import { Gift, GiftListResponse, GiftFilterParams, ApiResponse } from '@/types/gift';
...
class ApiService {
  ...
  async getPageGifts(params: Partial<GiftFilterParams> = {}): Promise<ApiResponse<Gift[]>> {
    ...
    if (response.success) {
      // Добавляем обработку и валидацию данных
  ...
  async getAllAvailableGifts(maxPages: number = 10): Promise<ApiResponse<Gift[]>> {
    const allGifts: Gift[] = [];
    let currentPage = 1;
    ...
    try {
      while (hasMoreData && currentPage <= maxPages) {
        const response = await this.getPageGifts({ page: currentPage, limit: 50 });
  ...
  // Метод для получения статистики по подаркам
  async getGiftsStatistics(): Promise<ApiResponse<any>> {
    try {
      const response = await this.getPageGifts({ limit: 1 });
      
      if (response.success) {
        // Здесь можно добавить логику для расчета статистики
        return {
          success: true,
          data: {
            totalAvailable: 0, // Нужно будет получить из API или расчетов
            averagePrice: 0,
